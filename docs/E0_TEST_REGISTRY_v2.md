...
**Current C330 collection:** **6842 tests** across **164 test files**; the two new tests were exercised in the focused suite, not yet in another full-suite run.
...
**C330 scope:** semantics-preserving G1 scaling remediation: persistent fields, topology caches, sparse-direct/CG solver parity, and Development-only full-budget timing evidence.
...
- The remediated workflow installs `.[science]` explicitly and records the
  SciPy version; a new complete development rerun is required before holdout.
...
✅ The first bounded development execution and its negative result are
confirmed for `af2ae45`. C330 has local semantic-parity and worst-case episode
timing evidence, but its full 1,560-replicate development rerun is pending.
**No holdout access and no Gate-G1 result exist.**
...
| **Current collected total (authoritative; not the sum of historical rows)** | **6842** | **164 files; last full execution: 6840** |
