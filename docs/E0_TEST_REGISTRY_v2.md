...
**Last full execution:** 2026-07-29 — **6840 tests** (6838 passed, 2 skipped, 0 failed) across **164 test files**
**Current C332 collection:** **7166 tests** across **181 test files** (unchanged since WP-GATE-0.15); the 326 tests added since the last full execution were exercised in focused suites, not yet in another full-suite run.
**Count authority:** `py -3 -m pytest e0_controller server -q -p no:cacheprovider`; older per-file rows below are historical and are not an additive source of truth.
**WP-GATE-0.16 scope:** Closes the external audit findings in governance documents only: Evidence Ledger coverage, a labeled preregistration erratum, and current bootstrap state. It adds no tests, changes no frozen instance or execution semantics, and observes no fresh-split outcome.
**C332 scope:** Retains the verified WP-2.4 development bundle (run 30526724307) before artifact expiry and reports all three preregistered development diagnostics, including the previously unreported adverse G1-B baseline comparison. No new tests, no holdout access, no gate result.
**C333 scope:** Records the user's closure decision: Gate G1-v1 CLOSED-NEGATIVE on development evidence, holdout permanently unopened, geometry research freeze (P4b), WP-2.5 closed unexecuted, Override-Gate v2 permanently parked. Governance documents only; no tests, code, or outcomes changed.
**C334 scope (WP-5.1):** README claims-to-ledger rewrite — negative G1 result reported up front, geometry marked research-frozen, override-gate and historization claims scoped to ledger entries, counts corrected, AGI blueprint moved to `_archive/canon/`. Documentation only; no tests or code changed.
**C335 scope (WP-5.2):** Closure paper draft `docs/papers/PAPER11_CLOSURE_MANUSCRIPT_v1.md` — preregistered negative result, mechanism diagnosis, wall_grid niche, and methodology, all numbers sourced from retained artifacts. Submission is a USER DECISION. Documentation only.
**C336 scope (WP-6.1):** Preregisters `E0-WP6-RELMEM-v1` — the final reliability_memory decision experiment (3 regimes × 30 paired seeds × 4 arms, frozen criteria incl. the sticky-heuristic comparator, shipped defaults only, PASS→maintained library / FAIL→archive). NOT implemented, NOT executed; WP-6.2 requires user review of the design. Documentation only.
**C337 scope (WP-6.2a):** Implements the E0-WP6-RELMEM-v1 harness (`wp6_relmem_experiment.py`) after user approval of the design: seeded environment with three regimes, four arms, frozen statistics and criteria, SHA-256-bound object under test, artifact writer. 22 no-outcome contract tests (`test_wp6_relmem_experiment.py`); no preregistered outcome produced. Collection: 7,188 tests / 182 files.
**C338 scope (WP-6.2b):** Executes all 360 preregistered replicates at execution commit 16929d4e (C337). Verdict PASS: +48–54 % over memoryless selection, +2.6 % stationary / +5.0 % drift over the sticky heuristic; context-keyed sticky beats the context-free store by 19.2 % (documented guidance). Mandatory sticky note applied to lean/E0_LEAN_CORE.md and README. Artifacts with SHA-256 manifest under `artifacts/wp6/E0-WP6-RELMEM-v1/`; deterministic re-run byte-identical. Closure arc ends in the maintained-library branch.
**C339 scope (WP-5.2 v2):** Updates the closure paper with the executed WP-6 result (abstract, contribution 6, section 9, limitations, conclusion, data availability) and adds the arXiv LaTeX source under `docs/papers/arxiv/PAPER11_v1/`. Submission to arXiv (cs.AI) cleared by user decision of 2026-08-05. Documentation only.
**Scope:** Deterministic controller, phase/amplitude layer, G5 geometries, hybrid arbitration, historization, multi-goal behavior, topology scans, Born sampling comparison, multi-axis SU(2), curvature modulation, LLM context enrichment, K5 field-based escalation, MemOS persistence fidelity, B4 self-tuning meta-layer, Session orchestrator, **C37 residual tension + iterative control (Axiom A₀)**, **C38 E0Envelope + TransportRegime**, **C39 resonator-controller integration**, **C40 graduated overlap functional (M_H from Ontodynamics §3.4)**, **C41 stochastic exploration policy (Born warmup → exploit)**, **B4-S1 Landscape mutation API (Bridge 4 Structural Reflexivity)**, **B4-S2 Structural Mutation Infrastructure**, **B4-S3 Structural Tuning Cycle + Session.iterate() hook**, **B4-S4a Identity Invariant (goal-reachable + A₀-compliant + historization-continuous)**, Beipackzettel real-world validation, non-circular amplitude mass trap, ProvenanceLog evidence chain, live LLM provenance, **C42 4-Layer Model (trace_load/trace_quality/inertia_factor)**, **C43 Self-Graph (Selbstunterscheidung)**, **C44 Bootstrapper (structured spec → Landscape)**, **C45 LLM Adapter v2 (propose_domain_graph)**, **C46 Mode Controller (LEARN/EXECUTE/COMBINATION)**, **C47 Dual Reflection (self-graph diagnosis + meta-control)**, **C48 Canon Materialization (Ontodynamics → navigable Landscape)**, **C49 Reflexive Action (diagnosis → concrete landscape mutation)**, **C50 Stufe 4b Representation (reflexive journal + self-exposition)**, **C51 System-Level Integration (E₀ lernt E₀)**, **C52 Honest Self-Knowledge (CANON_PROCESS_MAP correction)**, **C53 Domain-Invariance Benchmark (10 domains, 1 controller)**, **C54 Raumzeit Coupling (closed vs coupled systems)**, **C55 Amplitude Benchmark (10 domains × 3 modes)**, **C56 Reflexive Edge Proposal (Historisierung informiert Topologie)**, **C57 Proactive Reflexion (Stufe 2)**, **C58 Reflexion Benchmark (10 domains × 3 Stufen)**, **C59 Integrated Reflexion (C49 + C57 unified)**, **C60 Multiverse (coupled systems, NoveltyGate, convergence/divergence)**, **C61 Cross-Domain Multiverse Benchmark**, **C62 Cross-Universe Reflexive Edge Discovery**, **C63 OVERLOADED Escalation (OI-triggered peer consultation)**, **C64 Gridworld Baseline Benchmark (E₀ vs A* vs Naive-Greedy)**, **C66 CouplingRouter (N>2 dynamic partner selection)**, **C67 Asymmetric Coupling (weight-based directional R₀)**, **C68 Coupling Self-Graph (Stufe-3 reflexion for coupling pipeline)**, **C69 Cross-Reflexion Benchmark (edge copying vs edge creation)**, **C70 OVERLOADED Benchmark (peer consultation × 10 domains)**, **C71 LLM Co-Cognition (2 LLMs coupled via multiverse)**, **C72 Chess Engine (strategic dimension navigation)**, **C73 Primitive Extensions (fully_connected + strategy_profile)**, **C74 Team Chess (multiverse team play)**, **C75–C81 Empirical Validation Phase (attractor universality, multi-attractor dynamics, transfer learning, convergence speed, asymmetric ρ, attractor prediction, focus narrowing — explore scripts + formal tests)**, **C82 Focus Narrowing in Controller (focus_k + peer bypass)**, **C83 Service Layer (SnapshotCodec, InputPipeline, PeerBridge, ServiceSession, SessionManager)**, **C84 API Gateway (FastAPI REST + WebSocket, 13 endpoints)**, **C85 Client (React + Cytoscape.js)**, **C86 Test Runner UI**, **C88 Keimzelle (graph-centric UI rebuild)**, **C89–C93 UI iteration (edge labels, path trail, peer interaction, mechanism indicator, scenarios)**, **C94 Observation Landscape (O-Landscape)**, **C95 Observation Controller (navigation + projection)**, **C96 Rendering Adapter (observation → wire format)**, **C97 Observation UI Integration (server + client wiring)**, **C98 Graduated Overlap in Greedy Loop (M_H closes last research question)**, **C99 Inertia Dampening in Greedy Loop (confused edges penalized)**, **C100 Modulation Benchmark (14 domains × 3 modes, overlap + inertia validated)**, **C101 Scoped Reflexion (historization-driven locality for edge proposals)**, **C102 Scoped Reflexion Controller Integration (scoped=True wired into integrated_reflexion pipeline)**, **C103 Scoped Reflexion Benchmark (10 domains × GLOBAL vs SCOPED, fresh degeneration validated)**, **C104 Emergent Locality (historization creates locality as emergent property)**, **C105 Adaptive μ (μ = |E|/|V| derived from topology)**, **C106 Corridor Scope (non-spherical locality)**, **C107 Scoped Cross-Reflexion (multi-agent locality, P5 §10.4 Q3)**, **C108 Asymptotic Tightness (non-uniform convergence, P5 §10.4 Q4)**, **C109 Dream Mode Phase 1 (edge fingerprints, functional equivalence, dream readiness)**, **C110 Dream Mode Phase 2 (DreamObserver, dream_cycle, feedback historization, P4 noise filtering)**, **C111 Dream Mode Phase 3 (bridge hypothesis generation, dream peer_fn, P2 acceleration, P3 self-correction)**, **C112 Dream Mode Phase 4 (explore_dream_mode.py end-to-end: 6 domains, equivalence precision, bridge proposals, self-correction, DL convergence)**, **C115–C120 Structural Entropy (forgetting as structural necessity: inscription threshold, anchor analysis, structural decay, controller integration, dream consolidation)**, **C121 Sleep–Wake Cycle (automatic dream trigger via T_s, SleepWakeCycle orchestrator)**, **C122 Ontodynamics Canon v2.0 (51 nodes, 93 edges, English IDs, implementation layer)**, **C122b German ID Cleanup**, **C122c Dead-End Sink Repair**, **C122d Epistemic Liveness (U=2, F=1 uniform, "Zweifel zuzulassen")**, **C123 Curriculum Navigator (hierarchical learning, equilibrium detection, scoped landscapes, historization transfer)**, **C124–C128 Language Learning (EN↔DE cross-domain translation: partial dictionaries, iterative bootstrap, Level-1/2/3 validation, neighborhood consistency)**, **C129–C132 Language Learning Evolution (seed expansion, enriched canon (FALSIFIED), bilingual LLM teacher)**, **C133 LLM Monolingual Teaching + Seedless Playground (binary YES/NO, position-based matching: 44/44)**, **C134 Bootstrapper as Monolingual Teacher (score 0–10 → bootstrap_landscape, NodeFingerprint matching: 9/44)**, **C135 WL Recursive Neighborhood Matching (WLNodeFingerprint, depth-2: 33/44 at 100% precision, seedless)**, **C136 Feature Engineering (9-dim Round-0: +trace_load, +min/max/median quality, 34/44)**, **C137 Hungarian Optimal Assignment (find_wl_node_equivalences_hungarian, 44/44 = 100%, BREAKTHROUGH)**, **C139 DreamObserver + Hungarian/WL Runtime Integration (node_equivalence_method, dream_cycle node-EQ, _update_dream_landscape_nodes, 10 tests)**, **C148 Parameter Registry (E0Config central defaults, 45+ fields, 15 modules wired)**, **C149 μ/α Unification (function-level defaults flow through DEFAULTS, inertia_alpha rename)**, **C150 Parameter Sensitivity (Self-Graph evaluates config choices, COMPONENT_PARAMS, suggest_perturbations)**, **C151 Honest Activation (amplitude/born excluded from Self-Graph in GREEDY mode, ALWAYS_ACTIVE_COMPONENTS)**, **C152 Hybrid Mode in Sensitivity (hybrid_mode/use_su2 in E0Config, run_trial forwarding, 3-regime comparison)**, **C153 SU(2) Perspective Diagnostic (perspective_check, PerspectiveReport, controller + DualReflectionReport integration, 35 tests)**, **C154 DreamObserver Node Bridge Pipeline (propose_node_bridges structural transfer, make_dream_peer_fn node fallback, SleepWakeCycle.wire_peer_fns, confidence_floor, 32 tests)**, **C155 Auto-Tuning (closed-loop parameter optimization: diagnose → perturb → evaluate → adopt, auto_tune(), apply_config(), Session.auto_tune(), 25 tests)**, **C156 Curriculum ↔ Sleep-Wake Integration (DreamObserver consolidation between curriculum turns, TurnResult.dream_consolidation, observer/consolidation_cycles params, 14 tests)**, **C157 Dream Equivalences → CouplingRouter (update_weights_from_dream auto-biases partner selection from dream quality signals, DreamWeightUpdate/DreamWeightReport, 15 tests)**, **C158 Perception Ontology (Gestalt primitives as learnable perception domain, 48 tests)**, **C159 Communication Intent (intent detection from E0 controller state, 42 tests)**, **C160 UI-Schema Emitter (Intent × Perception → UISpec, 32 tests)**, **C161 Human Feedback Loop (closes perception learning cycle, 30 tests)**, **C162 Human Communication PoC (demo_human_communication.py end-to-end, 12 tests)**, **C163 UI Renderer (stateless UISpec → HTML, 38 tests)**, **C164 Learnable Rendering (visual_pretraining.py, LLM-based rendering selection, memo persistence, 30 tests)**, **C165 Session Runner (e0_session.py unified end-to-end pipeline, 15 tests)**, **C166 Task-Aware Intents (landscape + trace data flows into UI, 12 tests)**, **C166b LLM-Derived Endpoints (start/goal states derived from task description, 9 tests)**, **C167 Infrastructure Hardening (conftest.py, CI config, bootstrap.json)**, **C168 Compatibility-Gated Dreaming (dream_compatibility, is_dream_compatible, DreamObserver compatibility_threshold, 15 tests)**, **C169 Compatibility Threshold Calibration (36 domain pairs, 9 domains, gap=0.410, threshold 0.6 confirmed, 0 new tests — exploration only)**, **C170 Partial Structure Matching (NEGATIVE — 0% overlap between compatible and incompatible best-5 WL distances, 0 new tests — exploration only)**, **C171 Asymmetric Teaching (training effect=0, topology dominates, EN_heavy vs EN_light identical fingerprints, variance ratio=1.000, scoped to LLM-bootstrapped, 0 new tests — exploration + research doc)**, **C179 N-Domain Mesh N=3 (EN+DE+ONTO cluster formation, compatibility gating, dynamic boundary, 14 tests)**, **C180 N-Domain Mesh N=5 (COOK+PROJ bootstrapped domains, dual cluster formation, ONTO bridge effect, 28 tests)**, **C181 Dream Transitivity (ONTO bridges incompatible clusters via transitive chains, compatibility threshold NOT transitive, 25 tests)**, **C182 Saturation Point (mesh scaling N∈{3,5,7,10,13,16,18}, quality collapse detection, 15 parametric templates, 34 tests)**, **C183 E2E Multiverse (full capability exercise across all 14 layers, 15-phase pipeline, 35 tests)**, **C184 Real-World Validation (Wikispeedia 17/20 + BPI 2017 8-step rework-loop avoidance, Δ dominates, 0 new tests — explorations)**, **C185 Traffic Simulation (multi-agent grid city, first domain where conservative interference measurably helps, conf=0.85 best, 0 new tests — exploration)**, **C185b River City (6×8 bridge topology, historization persistence trap exposed, overlay corrects +56%, commute simulation, 0 new tests — exploration)**, **C186 Epistemic Trust (self-calibrating doubt: trust(e)=exp(−staleness/τ_doubt), stability from confirmations/surprises, τ_base self-calibrating, delta_H_trusted=δ_H·trust, River City validation: ρ-decay already handles bridge trap — trust addresses different failure class, 22 tests)**, **C187 Surprise Dampening (half-weight inscription for surprising outcomes, experience classification, surprise_rate, surprise_edges, 18 tests)**, **C188 Adaptive Observation (adapt_from_experience closes feedback loop: volatile→enable dampening, stable→disable, exploratory→hold; observation project() exposes surprise_metrics, 19 tests)**, **C189 Shared Historization (cooperative knowledge sharing: shared Historization instance across agents, River City +128–244% coop gain, Gordian Trap escape — competition→cooperation, 14 tests)**, **C195–C199 Bootstrap Landscape (bootstrap.json as navigable E0 landscape, persistence cycle, confidence updates, LLM semantic validation, executable transitions)**, **C200 Canon × Bootstrap Multiverse (51 Canon + 41 Bootstrap nodes, 58 bidirectional bridges, 80% crossing rate, unified exploration across theory and practice, 19 tests)**, **C201 Ontodynamics v3.0 (63 nodes, 131 edges, levels 0–18: 12 new concepts incl. transition_potential, epistemic_trust, auto_tuning, bootstrap_landscape, compatibility_gating, wl_node_fingerprint, n_domain_mesh, canon_bootstrap_multiverse; canon coverage 45/63=71.4%; unified landscape 104 nodes, 301 edges, 93.3% crossing rate; EN↔ONTO now dream-compatible; trivial loop fix epistemic_trust→inscription_threshold, 11 new tests)**, **C202 Learning Cycle (iterative self-improvement: ASSESS→PLAN→NAVIGATE→VALIDATE→CONSOLIDATE outer loop, exploration bonus for unvisited targets, frontier-adjacent start, 71.2%→96.2% coverage in 4 rounds, Canon 57.1%→98.4%, T_s halved, 34 tests)**, **C203 Learning State Separation (discovered_edges + cross_domain_bridges + learning_history → learning_state.json, bootstrap.json 1422→662 lines, 3 production modules + 2 test files rewired, 0 new tests — refactoring only)**, **C204 Multi-Domain Learning Cycle (EN as third domain: Canon 63 + Bootstrap 41 + EN 44 = 148 nodes, 470 edges, 42 EN bridges, explore_en mode, 54.1%→82.4% coverage, EN 13.6%→81.8%, 291 domain crossings, 38 tests)**, **C205 Edge Metadata in Landscape (_metadata: Dict[Edge, dict] on Landscape, **metadata kwargs on add_edge, edge_meta/set_edge_meta accessors, metadata survives full pipeline: canon_loader → bootstrapper → Landscape, 470/470 edges enriched, 12 relation types + 2 bridge types, 26 tests)**, **C206 Typed Navigation (RELATION_TYPE_BONUS 10 types: enables=1.4→opposite_of=0.85, BRIDGE_TYPE_BONUS: en_semantic=1.25/static=1.15, _edge_type_bonus wired into navigate() scoring, type_usage tracking in MultiDomainRoundResult, coverage 82.4%→84.5%, EN↔Canon crossings 75→104, 23 tests)**, **C207 Contextual Inscription (InscriptionContext dataclass: tau/edge/outcome/mode/relation_type/bridge_type/source_domain/target_domain/role/revisit_count/step, inscribe() on Historization, edge_inscriptions/inscription_summary/inscription_stats query methods, controller greedy + multidomain navigate wired, 176 inscriptions in 4-round run: 129 bridge + 45 exploration + 2 revisit, 31 tests)**, **C208 Text Renderer (render_text/render_markdown parallel to render_html, 7 text renderers: heatmap→bar, tree→indented, timeline→markers, bar→ASCII, text→paragraph, highlight→boxed, dashboard→metrics, urgency→emphasis mapping ‼/⚠/▸, _TEXT_RENDERERS dispatch dict, render_to_text_file, 57 tests)**, **C209 Evidence Interpreter (template-based prose from structured evidence dicts, 8 interpreters: uncertainty/decision/pattern/self_graph/task/dead_end/dream/generic, signature-based dispatch on evidence keys, interpret_inscription_summary + interpret_inscription_stats + interpret_trace + interpret_domain_crossings + interpret_panel, no LLM — structure IS the story, 58 tests)**, **C210 Session Text Mode (run_session output_format=html|text|markdown, E0SessionResult.output_path/output_format/text_output, --format CLI with md alias, rendering dispatch: text→render_text+interpret_panel, markdown→render_markdown+interpret_panel, html→render_and_open, backward-compat html_path property, 23 tests)**, **C211 Output Polish (drop_pct fraction→percentage fix in _interpret_pattern, evidence deduplication: _EVIDENCE_IN_VISUAL skips Evidence block for dashboard/tree/timeline/heatmap, _truncate_label with _LABEL_MAX_LEN=80, interpreter threshold calibration: >0.5/0.2/0.05 fractions, 14 new tests)**, **C212 Learning Cycle Communication (detect_round_intents: round data → 7 intent types: decision/coverage/balance/crossings/T_s/stagnation/shortcuts, communicate_round + communicate_summary wire full pipeline: intents → UISpec → render + interpret, --format text|markdown|md CLI on explore_learning_cycle_multidomain, 26 new tests)**, **C213 Interactive Text Session (REPL on real multi-domain landscape: SessionState persistent across interactions, 6 commands: run [N]/status/focus <domain>/why/summary/help, dispatch() parser with domain aliases c/boot/en/english, build_session + cmd_run/cmd_status/cmd_focus/cmd_why/cmd_summary, full communication pipeline per command, --format text|markdown CLI, 36 tests)**, **C214 Feedback Loop (rate command with aliases: helpful/good/not helpful/bad/confused/?, session-scoped Perception (seed from perception_pretrained.json), HumanAction historization, _RATING_ACTION dispatch, 28 tests)**, **C215 Interactive Browser Session (local HTTP server with HTML panel rendering, inline feedback buttons via rateFeedback() JS, command input bar + quick-action buttons, PRG pattern, XSS prevention, make_handler factory, 21 tests)**, **C216 Transition Detail (cmd_detail: edge-by-edge path view with quality/load/inertia per transition, CROSS tags for domain crossings, markdown table mode; cmd_inspect: deep edge narrative with inscription history, trace metrics, success rate, recent InscriptionContext entries; _quality_bar ASCII visualization, 27 tests)**, **C217 Human Peer Input (cmd_task: free-text difference → structural matching via _match_nodes token↔nodeID overlap, connectivity analysis between matched nodes, anchor-based navigation with mode=task, structural gap signal for LLM peer C218 hook; 9 TestMatchNodes + 10 TestCmdTask + 5 TestTaskDispatch + 1 help assertion, 25 tests)**, **C218 LLM Peer Structuring (when _match_nodes returns 0: _get_llm_adapter lazy-init → propose_domain_graph → _inject_spec_into_landscape with T: prefix + _apply_confidence traces → _create_bridges bidirectional to existing nodes via concept overlap → navigate from anchor with mode=task_llm; graceful error fallback; 11 TestLLMPeerStructure + 3 TestLLMPeerError + 5 TestInjectSpec, 19 tests)**, **C219 Semantic Surface + 3-Tier Task Processing (description field on all 148 unified_nodes from Canon/Bootstrap/EN sources; _match_nodes searches labels+descriptions not just IDs; three-tier routing: Tier 1 Known Concept (single ≥0.8 match → neighborhood + navigate), Tier 2 Connection (2+ matches → check/create human_structural edges → navigate), Tier 3 LLM Peer (0 matches → C218); free-text dispatch (no 'task' prefix); _task_navigate shared helper; 5 TestSemanticSurface + 5 TestMatchNodesWithDescriptions + 6 TestThreeTierRouting + 3 TestTaskKnownPath + 5 TestTaskConnection + 3 TestTaskNavigate, 27 tests)**, **C220 Self-Knowledge Seed (E₀ learns itself to ≥98% coverage, exports as warm-start JSON; learn_self 3-phase: standard cycle → targeted passes → direct inscription for unreachable nodes; export_seed full landscape snapshot + unified_nodes + edge_meta; load_seed restores traces/metadata; build_session warm start with self_knowledge_path; snapshot_codec fix: _encode_historization_dict handles confirmations/surprises Edge keys; early stagnation exit after 10 rounds; 7 TestLearnSelf + 1 TestEarlyExit + 7 TestExportSeed + 6 TestLoadSeed + 3 TestBuildSessionWithSeed + 2 TestHelpers + 2 TestSnapshotCodecFix, 27 tests)**, **C221 Mechanism Domain (M: — E₀ learns HOW it works: 20 mechanism nodes across 8 derivation levels, 43 typed intra-edges: enables/feeds/triggers/constrains, 108 cross-domain bridges: M↔Canon + M↔Bootstrap, 4-domain unified landscape: 168 nodes/621 edges, MultiDomainAssessment + navigate + consolidate wired for M:, per-domain mech_coverage/mech_visited/mech_nodes tracking, 6 crossing pair types, 1 TestDomainOf + 7 TestMechanismDomain + 4 TestMechBridges + 4 TestMechIntraEdges, 15 tests)**, **C222 PC Orphan Fix (3 GT→PC + 7 PC→WP edges, bootstrap max coverage 92.7%→100%, 631 edges, 0 new tests)**, **C223 Focused Seed (EN vocabulary optional via include_en parameter, GT-2/WP-5/WP-6 topology fixes, SelfKnowledgeResult.mech_coverage, mech crossing data in learn_self, export_seed with mech_coverage, 3-domain seed C+B+M: 124 nodes/100% coverage, 1 new test: test_mech_coverage, 1 updated: test_en_coverage→test_en_coverage_excluded)**, **C225 Session Persistence (save_session/load_session round-trip via snapshot_codec, cmd_save dispatch, auto-save on quit/Ctrl+C, build_session fallback chain: session_state→seed→cold start, auto_detect=False default (GT-6), consolidate dry_run=False, SESSION_STATE_PATH, 5 TestSaveSession + 7 TestLoadSession + 2 TestCmdSave + 1 TestSaveInHelp + 2 TestAutoDetect + 1 TestConsolidateNotDry, 18 tests)**, **C226 Session History + Server Lifecycle (history serialization as lightweight dicts: _round_to_dict/_dict_to_round/_assessment_to_dict/_dict_to_assessment codec, save_session persists full history, load_session restores MultiDomainRoundResult objects, summary/detail/why work after reload, server auto-save on Ctrl+C via save_session in run_server finally block, perception write-back: write_back_perception=True opt-in updates perception_pretrained.json, version 1.0→1.1, backward-compat for v1.0 files without history key, 10 TestHistoryRoundTrip + 2 TestRoundDictCodec + 4 TestPerceptionWriteBack + 1 TestServerAutoSave, 16 tests)**, **C227 Seed Regeneration (regenerate_seed: materialize qualifying discovered_edges from learning_state.json into landscape with confidence gating ≥0.4, re-assess coverage, export enriched seed via export_seed; cmd_regenerate dispatch + help; multi-session learning loop: Session A→save→Session B→run→regenerate→Session C starts with A+B knowledge; 5 TestRegenerateSeed + 4 TestRegenerateMaterialization + 3 TestCmdRegenerate + 1 TestMultiSessionLoop, 13 tests)**, **C228 Observation Dashboard (compute_trajectory: structured per-round metrics with domain trends and mode progression; diagnose_session: per-domain stagnation analysis with status classification SATURATED/BLOCKED/STAGNANT/GROWING/IDLE, bottleneck identification, velocity tracking from recent history, confused edge detection, escalation hints; cmd_trajectory text+markdown table output; cmd_diagnose formatted report with overall diagnosis; dispatch routing for trajectory+diagnose; 5 TestComputeTrajectory + 5 TestCmdTrajectory + 5 TestDiagnoseSession + 5 TestCmdDiagnose, 20 tests)**, **C229 Stagnation Escalation (escalate: 5-level progressive response — focus_shift/exploration_boost/bridge_creation/edge_proposal/accept, coverage_delta measurement per level, auto-trigger in cmd_run at stagnation≥3, cmd_escalate manual trigger, dispatch routing; 5 TestEscalate + 5 TestCmdEscalate + 2 TestEscalationDispatch + 3 TestAutoEscalation, 15 tests)**, **C230 Teaching Pipeline (teach_concept: LLM structuring → L: prefix injection → multi-pass exploration with persistent consolidation, absorption tracking via trace_load on new edges; cmd_teach formatted output with nodes/coverage/absorption report; dispatch routing + HELP_TEXT; 7 TestTeachConcept + 5 TestCmdTeach + 3 TestTeachDispatch, 15 tests)**, **C231 Session Journal (record_journal_event: event-based journal with automatic _metrics_snapshot, event types session_start/round/teach/escalate/note/session_end; save_journal/load_journal: cross-session accumulation in JSON with deduplication by timestamp+session_id+event_type+round_num; cmd_journal: current-session view, cross-session trajectory, human annotations; _format_journal: event icons ▶●📖⚡✎■, coverage trajectory summary; wired into build_session/cmd_run/cmd_teach/cmd_escalate/save_session; dispatch routing + HELP_TEXT; 4 TestMetricsSnapshot + 6 TestRecordJournalEvent + 5 TestSaveLoadJournal + 7 TestCmdJournal + 2 TestSessionIdField + 3 TestJournalDispatch, 27 tests)**, **C232 Meta-Reflection (meta_reflect: synthesizes trajectory + domain diagnosis + journal events to identify systematic stagnation patterns: stagnation episodes, mode effectiveness with avg_delta/stagnation_rate/crossings, domain trajectories with coverage/status/velocity/confused_edges, escalation/teaching summaries, pattern identification: persistent stagnation/mode ineffectiveness/blocked domains/confused edges/escalation exhaustion/coverage plateau, actionable recommendations; cmd_reflect formatted output with mode/domain/episode/intervention/pattern/recommendation sections; 'reflect' journal event; dispatch routing + HELP_TEXT; 12 TestMetaReflect + 2 TestMetaReflectStagnation + 7 TestCmdReflect + 2 TestReflectDispatch, 23 tests)**, **C233 Curriculum Command (curriculum_run: structured hierarchical learning via CurriculumRunner wired into interactive session, prefix-aware historization transfer _transfer_to_session maps raw IDs to C:/M:/EN: prefixed session edges, journal event with turn count/steps/equilibrium/transferred_edges; cmd_curriculum: formatted output with per-turn details + session coupling summary, partial canon matching; AVAILABLE_CANONS: ontodynamics/mechanism_e0/english_basic_enriched; dispatch routing + HELP_TEXT; 7 TestCurriculumRun + 8 TestCmdCurriculum + 3 TestCurriculumDispatch + 4 TestAvailableCanons, 18 tests [note: 4 TestAvailableCanons are simple assertions])**, **C234 Dream Command (dream_run: domain sub-landscape extraction _extract_domain_landscapes groups by C:/B:/EN:/M: prefix with historization copy, DreamObserver with Hungarian node equivalences + compatibility gating, N dream cycles, readiness report; cmd_dream formatted output: domain readiness scores, per-cycle equivalences (edge + node) with incompatible pair reporting, Dream Landscape stats; observer persists on SessionState.dream_observer; journal event with cycles/domains/equivalences; dispatch routing + HELP_TEXT; 4 TestExtractDomainLandscapes + 6 TestDreamRun + 8 TestCmdDream + 3 TestDreamDispatch, 21 tests)**, **C235 Sleep-Wake Integration (sleep_wake_run: per-domain E0Controllers from _extract_domain_landscapes, SleepWakeCycle with DreamObserver dream pressure trigger T_s/(T_s+μ)>0.5, N episodes with automatic wake/sleep phases, historization transfer back to session, _pick_domain_start intelligent start selection; cmd_sleep formatted output: episode results with wake outcome + sleep status, domain pressure report, summary; journal event with episode count/domains/slept; dispatch routing + HELP_TEXT; 8 TestSleepWakeRun + 8 TestCmdSleep + 3 TestSleepDispatch, 19 tests)**, **C236 Tune Command (tune_run: per-domain auto-tuning via auto_tune C155 — Self-Graph diagnosis → COMPONENT_PARAMS → ±perturbation variants → sensitivity_analysis → adopt best config, _extract_domain_landscapes domain isolation, _pick_domain_start start selection, meta_reflect pattern context; cmd_tune formatted output: domain results with quality initial/final/Δ + adopted parameter changes, summary, meta-reflection context; journal event with domains_tuned/improved/total_trials; dispatch routing + HELP_TEXT; 8 TestTuneRun + 8 TestCmdTune + 3 TestTuneDispatch, 19 tests)**, **C237 Auto-Mode (auto_run: autonomous learning loop orchestrating run/escalate/dream/sleep/curriculum/tune/stop, _choose_action decision tree: coverage/domain-status/stagnation/confused-edges → best action, max_steps budget, coverage tracking, journal event auto_mode with full action sequence; cmd_auto formatted output: action icons ▶/⚡/☁/☾/✉/⚙/■, per-step coverage, stopped reason; dispatch routing + HELP_TEXT; 4 TestChooseAction + 8 TestAutoRun + 8 TestCmdAuto + 3 TestAutoDispatch, 23 tests)**, **C238 Self-Learn (selflearn_run: E₀ learns itself first — orchestrates curriculum ontodynamics (WHAT) → curriculum mechanism_e0 (HOW) → dream consolidation (cross-domain equivalences) → self-mastery assessment (per-domain coverage + canon↔process alignment + readiness gate); _assess_self_mastery: C: and M: node coverage via trace_load, canon_self_bridge alignment ratio, overall score, ready threshold ≥0.5; cmd_selflearn formatted output: per-phase turn details, dream equivalences, coverage bars, mastery verdict; journal event selflearn with per-phase steps/transfers/mastery; dispatch routing + HELP_TEXT; 4 TestAssessSelfMastery + 7 TestSelflearnRun + 8 TestCmdSelflearn + 2 TestSelflearnDispatch, 21 tests)**, **C239 Ask Command (ask_run: on-demand question-answering pipeline — _extract_question_terms tokenization with stopword filtering, _assess_knowledge per-term structural coverage analysis against landscape nodes, gap detection for unmatched terms, auto_learn=True triggers teach_concept for up to 3 gap terms via LLM, re-assessment after learning, navigation from best match, confidence = term coverage ratio; cmd_ask formatted output: knowledge assessment with known/gap terms, on-demand learning results, navigation metrics, confidence verdict high/moderate/low; journal event ask with terms/gaps/coverage/confidence; dispatch routing + HELP_TEXT; 8 TestExtractQuestionTerms + 9 TestAssessKnowledge + 10 TestAskRun + 8 TestCmdAsk + 3 TestAskDispatch, 38 tests)**, **C242 Answer Synthesis (Phase 5 completes ask pipeline: SYNTHESIZE_ANSWER_PROMPT 9th prompt type in llm_adapter, synthesize_answer() method; _format_path_evidence deduplicates nodes, shows [domain] label: description + edge transitions with quality; _structural_answer no-LLM fallback listing up to 8 concepts; ask_run Phase 5 tries LLM synthesis → structural fallback; cmd_ask Answer section with key_concepts + evidence_sufficient warning; 6 TestFormatPathEvidence + 6 TestStructuralAnswer + 4 TestAskRunSynthesis + 4 TestCmdAskAnswerSection + 4 TestSynthesizeAnswer, 24 tests)**, **C253 Teach-Aware Ask (balanced scoring (node_ratio+query_ratio)/2, L:-anchor preference in ask_run; 5 TestMatchNodesBalancedScoring + 3 TestAskAnchorPreference, 8 tests)**, **C254 Dream Backflow + Bridge Resistance (_inject_dream_bridges: dream node-equivalences injected as navigable cross-domain edges R=0.35 into session landscape after dream_run; _create_bridges R reduced 1.2→0.4, delta 0.4→0.3; dream_bridges_added in result/journal/cmd_dream output; 6 TestInjectDreamBridges + 3 TestDreamRunBackflow + 2 TestBridgeResistanceReduced, 11 tests)**, **C255 Community Detection from R_eff (community.py: detect_communities(landscape) → List[Set[str]] using weighted Label Propagation Algorithm on 1/R_eff edge weights; emergent structure from Historization replaces prefix-based domain partitioning; cold start = connected components; navigation creates clusters; deterministic (sorted nodes, smallest-label tiebreaker); no external deps — built from E₀ primitives only; 6 TestCommunityBasics + 8 TestCommunityTopology + 4 TestCommunityHistorization + 6 TestCommunityProperties, 24 tests)**, **C256 Dual-Mode Validation (NMI comparison: emergent communities vs prefix-based domains; normalized_mutual_information arithmetic-mean NMI; extract_community_landscapes parallel to legacy _extract_domain_landscapes with actual Δ/R₀; prefix_partition from ^[A-Z]+: regex; compare_partitions end-to-end: verdict aligned/partial/divergent; wired into diagnose_session + cmd_diagnose + cmd_status; 11 TestNMI + 8 TestExtractCommunityLandscapes + 5 TestPrefixPartition + 5 TestComparePartitions, 29 tests)**, **C257 Dream on Communities (dream_run partition='community'|'prefix' default community; community mode uses detect_communities→extract_community_landscapes instead of prefix-based _extract_domain_landscapes; cmd_dream 'dream [community|prefix] [N]' arg parsing; _inject_dream_bridges fixed for community domains: node names already fully qualified, no prefix prepending; cross-universe L: extraction remains prefix-based; auto_run + selflearn_run inherit community default; 8 TestDreamRunPartition + 9 TestCmdDreamPartition + 3 TestDreamCommunityBridges, 19 tests [note: 1 test added to existing TestDreamCommunityBridges])**, **C263 Cold Start Alignment (build_multidomain_landscape default include_en=True→False; cold start matches warm start: C+B+M only, EN available as opt-in; existing callers updated to explicit include_en=True where needed; test_has_three_domains→expects 0 EN nodes; focus/alias tests updated for absent EN domain; 7 TestColdStartAlignment new tests, 758 passed total in test_interactive_session.py)**, **C264 Structural Bridges for Teach (_create_bridges uses find_structural_resonance WL-Hungarian for >=3 new nodes; lexical word-overlap fallback for <3 nodes; bridge_type='structural_resonance' tag; same delta=0.3 R=0.4 as C254; _lexical_bridge_fallback extracted; unbridged nodes get lexical fallback; 9 TestStructuralBridges, 767 passed total in test_interactive_session.py)**, **C265 Automatic Community Refresh (SessionState.communities field caches detect_communities result; refresh_communities() called after build_session/cmd_run/teach_concept/selflearn_run; all nodes covered invariant; topology mutation updates partition; 7 TestCommunityRefresh, 774 passed total in test_interactive_session.py)**, **C266 Community Crossings (navigate() crossing detection uses community_of() membership check instead of _domain_of() prefix comparison; community_of(node, communities)→int helper; community_crossings field on MultiDomainRoundResult; crossing_rate computed from community boundaries; navigate() accepts optional communities parameter — fallback to prefix when None; community_crossings ≤ domain_crossings invariant; cmd_detail shows 'community crossings'; all 8 navigate() call sites pass state.communities; backward compat: domain_crossings field + per-pair counts preserved; 10 TestCommunityCrossings, 784 passed total in test_interactive_session.py)**, **C267 Single Partition World (DeprecationWarning on all prefix-based fallbacks: navigate() without communities, cmd_detail/cmd_inspect without communities; all production decision paths use community_of() membership; _domain_of() remains valid for display labels only; module-level import warnings in interactive_session.py; navigate/cmd_detail/cmd_inspect community-first with explicit else-fallback deprecation; capstone integration test exercises full pipeline under warnings.simplefilter("error", DeprecationWarning); 10 TestSinglePartitionWorld, 794 passed total in test_interactive_session.py)**, **C269 Paper-Code Consistency (Phase A audit remediation: F1–F8 formula/default fixes across P1/P5/P6 + code, test_paper_defaults.py: 17 regression tests guarding paper-stated parameter defaults — α=2.0, k=3, ρ=0.9, λ_s=0.15, λ_f=0.20, δ_max=3.0, inertia_alpha=0.5, μ=|E|/|V| topology-derived, NoveltyGate θ=0.0; emergent_locality.py μ defaults changed from DEFAULTS.mu to None; 6 TestPaper1Defaults + 7 TestPaper5Defaults + 1 TestPaper6Defaults + 3 TestHistorizationDefaults, 17 new tests)**, **C277 Trajectory-Level Historization (PathSignature = compressed community-index tuple, domain-invariant; TrajectoryHistorization: U/F traces on PathSignatures, trace_load/trace_quality/low_quality_warning; TrajectoryRecord.outcome: productive/improving/stagnant from coverage_delta; compute_path_signature with consecutive dedup; plan() gains non-Markov trajectory signal — proactive mode switch before stagnation; run_multidomain_cycle wired with community-aware signature computation; MultiDomainRoundResult.trajectory field; BT-5 breakthrough; 8 TestPathSignature + 11 TestTrajectoryHistorization + 6 TestTrajectoryOutcome + 5 TestPlanWithTrajectory = 31 new tests — C277)**, **C278 Trajectory Experience Classification (trajectory_surprise_rate(): global fraction of trajectory revisits contradicting quality prediction, analogue of Historization.surprise_rate(); classify_trajectory_experience(): 'stable'/'volatile'/'exploratory' three-state classification, same vocabulary as edge-level classify_experience(), exploratory guard requires ≥3 revisit events; inscribe() extended with prediction tracking on revisits: confirmation when predicted_productive == actual_productive, surprise on mismatch; first inscription never tracked — no prior exists; 7 TestTrajectorySurpriseRate + 6 TestClassifyTrajectoryExperience = 13 new tests)**, **C279 Trajectory Experience Benchmark — Stage 1 Synthetic Validation (benchmark_trajectory.py: RegimeResult + SignatureStats + TrajectoryBenchmarkResult dataclasses, run_all_benchmarks() drives three synthetic session regimes; stable regime: 2 sigs × 8-consistent + 2-flip inscriptions → 14 CONF + 4 SURP → rate=0.222 → 'stable'; volatile regime: 20 alternating inscriptions on single sig → 9 CONF + 10 SURP → rate=0.526 → 'volatile'; exploratory regime: 4 distinct sigs × 1 inscription → 0 events → 'exploratory'; all 3 regimes correctly classified; key design constraint: 2-community landscapes → ≤4 distinct signatures → reliable revisit accumulation in ≤20 rounds (birthday paradox); TestTrajectoryEndToEnd integration class (C279): test_exploratory_with_single_revisit_event + test_leaves_exploratory_after_sufficient_data + test_stable_session_pattern (rate=0.25) + test_volatile_session_pattern (rate=0.75) + test_two_community_signature_diversity (20-round cycling over 4 sigs → not exploratory) = 5 new tests, 49 total in test_trajectory.py)**, **C280 Trajectory Adaptation (adapt_from_trajectory_experience() on TrajectoryHistorization: mirrors C188 adapt_from_experience() at trajectory level; maps classify_trajectory_experience() → plan() parameters: volatile → quality_threshold=-0.15 + step_multiplier=1.5 (sensitive + moderate), stable/exploratory → quality_threshold=-0.3 + step_multiplier=2.0 (defaults + decisive); plan() updated: adaptation fetched before low_quality_warning, steps=int(base_steps×multiplier), reason includes experience label; key discrimination: quality≈-0.167 fires volatile threshold but not stable threshold; TestTrajectoryAdaptation (C280): test_adapt_exploratory_returns_defaults + test_adapt_stable_returns_defaults + test_adapt_volatile_returns_sensitive_threshold + test_plan_volatile_fires_at_intermediate_quality + test_plan_stable_does_not_fire_at_intermediate_quality + test_plan_volatile_uses_1_5x_steps + test_plan_stable_uses_2x_steps = 7 new tests, 56 total in test_trajectory.py)**, **C281 Trajectory Integration Audit (trajectory_hist wired into interactive session pipeline: TrajectoryHistorization added to SessionState as default_factory field; cmd_run now passes trajectory_hist to plan() for adaptive thresholds + computes PathSignature via compute_path_signature + creates TrajectoryRecord + calls inscribe() after each round; _trajectory_hist_to_dict/_trajectory_hist_from_dict serialization helpers (tuple keys as JSON arrays + ast.literal_eval restore); save_session persists trajectory_hist; load_session restores it with backward-compat (missing key → fresh instance); audit finding: trajectory_hist was fully functional in run_multidomain_cycle() but absent from interactive session path — C281 closes the gap; TestTrajectoryIntegration (C281): test_session_state_has_trajectory_hist + test_cmd_run_inscribes_trajectory + test_cmd_run_accumulates_across_rounds + test_classify_trajectory_after_runs_is_valid + test_trajectory_hist_round_trip + test_load_session_backward_compat_no_trajectory_key + test_trajectory_hist_codec_inverse = 7 new tests)**, **C283 Stable Quality Profile (stable_quality_profile() — second axis of (predictability × quality) profile: closes 'Stable Underdetermination' open thread from C282; stable_quality_profile() computes mean trace_quality of known signatures and returns 'productive'/'stagnant'/'unknown'; adapt_from_trajectory_experience() extended to use both axes: stable+stagnant → quality_threshold=-0.1 + step_multiplier=3.0 (aggressive intervention for chronic traps), stable+productive → unchanged defaults -0.3/2.0, volatile → unchanged -0.15/1.5; result dict gains 'experience' + 'quality_profile' keys; canonical example: (0,3,0) structural pendulum from C282 → stable+stagnant → 3.0x steps; TestStableQualityProfile (C283): test_unknown_when_empty + test_stagnant_when_all_stagnant + test_productive_when_all_productive + test_productive_when_mean_quality_positive + test_stagnant_when_mean_quality_nonpositive + test_min_load_filters_low_load_signatures + test_adapt_returns_experience_key + test_adapt_returns_quality_profile_key + test_adapt_stable_stagnant_uses_aggressive_defaults + test_adapt_stable_productive_uses_default + test_adapt_volatile_unchanged + test_adapt_exploratory_returns_defaults + test_canonical_trap_pattern_uses_aggressive_defaults = 13 new tests, 83 total in test_trajectory.py)**, **C282 Trajectory Session Benchmark — Stage 2 real-world validation (benchmark_trajectory_session.py: SignatureProfile + SessionBenchmarkResult with signature_diversity/collision_rate/escalation_fired observables; run_session_benchmark(rounds, steps_per_round) runs build_session + cmd_run and extracts all trajectory observables; Stage 2 finding: 3 communities in real session, collision_rate=0.733, experience='stable', escalation fired 10/30 rounds on (0,3,0) pattern — trajectory adaptation IS live in real sessions; TestSessionBenchmark (C282): 14 structural tests covering result shape, invariants (unique_signatures ≤ rounds_run, diversity/collision complement, revisit_events consistency), adaptation dict structure, escalation consistency = 14 new tests, 70 total in test_trajectory.py)**.
...
| `test_g1_domains.py` | WP-2.1 infrastructure only: exact-N domain invariants, determinism, artifact contract, holdout protection |
| `test_g1_baselines.py` | WP-2.2 method registry, information boundaries, fair budgets, algorithms, 960-cell compatibility, holdout protection |
| `test_g1_ablations.py` | WP-2.3 five-level causal contract, shared B–E path family/gate, fair budgets, 600-cell compatibility, holdout protection |
| `test_g1_development_report.py` | WP-2.4 full-budget task grid, fresh-process workers, resume validation, artifact bundle, control selection, paired bootstrap, holdout protection |
| `test_g1_development_distributed.py` | WP-2.4 stable 240-batch partition, hard process timeout, timeout shard shape, atomic persistence, incomplete-consolidation rejection |
| `test_g1_timeout_enforcement.py` | WP-2.4 60-second decision-boundary deadline, ablation propagation, whole-replicate valid-negative score |
| `test_g1_mechanism_diagnosis.py` | C331 Development-only gate-blocker accounting, pairwise mechanism divergence, evidence aggregation, threshold reachability, holdout protection |
| `test_override_gate_policy.py` | WP-GATE-0.2 policy validation, exact legacy mappings, live gate behavior, Envelope/MemOS backward compatibility |
| `test_override_gate_calibration.py` | WP-GATE-0.3 frozen instance, scope, candidate grid, split separation, risk/selection boundaries, planned counts, semantic-drift rejection |
| `test_override_gate_calibration_runner.py` | WP-GATE-0.4 task/matrix determinism, control pairing, lifecycle authorization, artifact contracts, dry-run seal, no-domain/no-run boundaries |
| `test_override_gate_calibration_engine.py` | WP-GATE-0.5 calibration namespace/invariants, exact candidate enforcement, gate isolation, complete-state paired replay, parent non-mutation, path-cap closure |
...
| `test_dream_causal_transfer.py` | C178 |
...
| `test_shared_historization.py` | C189 (shared historization: cooperative knowledge sharing, collective vs individual volatility, shared R_eff propagation, River City +128–244% coop gain) |
...
| `test_perspective_diagnostic.py` | C153 (SU(2) perspective check) |
| `test_dream_node_bridges.py` | C154 (DreamObserver node bridge pipeline) |
| `test_curriculum.py` | C123/C156 (curriculum navigator, sleep-wake integration) |
| `test_perception.py` | C158 (perception ontology, Gestalt primitives) |
| `test_communication.py` | C159 (communication intent detection) |
| `test_ui_emitter.py` | C160 (UI-schema emitter) |
| `test_feedback.py` | C161 (human feedback loop) |
| `test_human_communication_integration.py` | C162 (human communication PoC) |
| `test_ui_renderer.py` | C163 (UISpec → HTML renderer) |
| `test_visual_pretraining.py` | C164 (LLM-based rendering selection) |
| `test_e0_session.py` | C165/C166 (session runner, task-aware intents) |
| `test_context_sensitivity.py` | C176 (context sensitivity metric) |
| `test_demo_bootstrap.py` | C140 (demo_bootstrap_domain smoke test) |
| `test_demo_curriculum.py` | C143 (demo_curriculum smoke test) |
| `test_demo_multiverse.py` | C142 (demo_multiverse smoke test) |
| `test_demo_reflexion.py` | C144 (demo_reflexion smoke test) |
| `test_demo_self_graph.py` | C43 (demo_self_graph smoke test) |
| `live_test_cocognition.py` | C71 (live, requires API key) |
...
**Claim**  
All components from C43–C50 work together as one system. The framework can operate on its own canon landscape (Selbst-Fundierung), accumulate self-knowledge through iterate(), trigger reflexive action within the full Session pipeline, and produce a 5-section self-exposition that coherently reflects what it believes, how it operates, what it covers, what remains open, and what it has done to itself.

**Evidence**  
- `e0_controller/test_system_integration.py` (19 tests, 5 classes)
- Full pipeline: Session → iterate → controller.cycle → self_graph.self_historize → dual_reflect → apply_reflexive_actions → journal.record → build_self_exposition
- Canon landscape (19 nodes, 31 edges) used as domain — E₀ navigates its own ontology
- Pre-poisoned curvature deactivated within iterate() Step 7 (not manually)
- All 5 exposition sections verified after real operation
...
### C55 — Amplitude Benchmark: 3 controller modes × 10 domains

**Claim**  
Amplitude-aware modes (AMPLITUDE_ON_DISAGREE, BORN_SAMPLING) are **not** domain-invariant. GREEDY remains the only mode reaching all 10 goals. However, amplitude mode improves trap-escape domains (D3, D4, D10) — fewer steps, better ratings.

**Evidence**  
- `e0_controller/benchmark_amplitude.py` — full 3-mode benchmark
- `e0_controller/test_amplitude_benchmark.py` (23 tests, 5 classes)
- GREEDY: 10/10 goals, worst=B (confirms C53)
- AMPLITUDE_ON_DISAGREE: 8/10 goals, D5 (grid) and D8 (nested cycles) fail
- BORN_SAMPLING (20 trials): D5 only 15% reach, D8 85% reach
- Amplitude improves D3, D4, D10 (A rating vs B under greedy)
- D5 failure: amplitude overrides grid navigation into dead ends
- D8 failure: amplitude persistently overrides toward cycle (16+ overrides)
- Born sampling shows rating variance across trials

**Result**  
- GREEDY dominates on goal-reach (10 vs 8)
- AMPLITUDE dominates on step-efficiency where it succeeds (≤ steps in 6/8 domains)
- Trade-off: amplitude override exchanges completeness for efficiency on trap domains
- Grid topologies and cycle-heavy domains are amplitude-hostile
...
### C64 — Gridworld Baseline Benchmark: E₀ vs A* vs Naive-Greedy

**Claim**  
E₀'s historization-based navigation outperforms memoryless greedy on trap
topologies and matches A* optimal path length on cycle-dominated grids.
Three 5×5 grid variants systematically test this against baselines:
V1 (detour wall), V2 (dead-end lure), V3 (trap loop).

This is the first formal baseline comparison: same topology, same start/goal,
three algorithms. Naive-Greedy (no memory) fails 100% on all variants.
E₀ (historization + revisit penalty + escalation) succeeds 100%.
On V3, E₀ matches A* optimal (8 steps).

**Evidence**  
- `e0_controller/benchmark_gridworld.py` — 3 grid variants, A*, Naive-Greedy, E₀ runners
- `e0_controller/test_benchmark_gridworld.py` — 49 tests across 6 classes:
  - `TestGridConstruction` (10): cell naming, grid validity, wall placement, lure/trap deltas
  - `TestAStarBaseline` (7): A* finds optimal paths (8 steps all variants), disconnected graph → None
  - `TestNaiveGreedyFailure` (9): 0% success on all 3 variants, cycling confirmed
  - `TestE0Success` (6): 100% success all variants, step bounds ≤2×A*, V3=A*-optimal
  - `TestComparativeInvariants` (12): 5-trial runs, method-level success/fail/bound invariants
  - `TestBenchmarkRunner` (5): runner completeness, determinism, dataclass validity

**Result**  
- V1 (detour wall): A*=8, Naive=0%, E₀=100%/16 steps (2× optimal — detour cost)
- V2 (dead-end lure): A*=8, Naive=0%, E₀=100%/10 steps (1.25× optimal)
- V3 (trap loop): A*=8, Naive=0%, E₀=100%/8 steps (A*-optimal — historization breaks cycle immediately)
- E₀ is deterministic: min_steps == max_steps across all trials
...
### C72 — Chess Engine: Strategic Dimension Navigation

**Claim**  
E₀ can play chess by navigating a landscape of 6 strategic dimensions,
discovering which dimension transitions are productive through
historization alone — no LLM required during play.

**Evidence**  
- `e0_controller/chess_e0.py` — `ChessE0Player`, `play_game()`, 6 dimension evaluators, `best_move_for_dimension()`
- `e0_controller/test_chess_e0.py` — 26 tests across 5 classes:
  - `TestDimensionEvaluators` (9): material, king safety, center, activity, pawn structure, development, profile
  - `TestBestMoveSelection` (3): captures for material, development moves, always returns valid
  - `TestChessE0Player` (5): landscape structure, select_move, dimension recording, strategy summary
  - `TestSelfPlay` (6): game terminates, valid result, moves recorded, both colors, summary
  - `TestHistorizationLearning` (3): edges historized, quality varies, dimension variety

**Result**  
- Self-play: 60 full moves, drawn by threefold repetition
- CENTER_CONTROL dominates (12/60 for both sides) — discovered by E₀
- DEVELOPMENT→CENTER_CONTROL has trace_quality=+1.0 (always productive)
- MATERIAL→PAWN_STRUCTURE has trace_quality=−1.0 (never productive)
- Classical opening principle (develop then control center) emerges from historization
- 6 dimensions × 30 edges × full game in <3 seconds
...
### C73 — Primitive Extensions: Fully Connected + Strategy Profile

**Claim**  
Two new framework primitives derived from C72 Chess insights:  
(1) `Landscape.fully_connected()` for uniform-initialization landscapes,  
(2) `Historization.strategy_profile()` for extracting what E₀ learned.

**Evidence**  
- `e0_controller/landscape.py` — `Landscape.fully_connected(states, delta, resistance)` classmethod
- `e0_controller/historization.py` — `Historization.strategy_profile(edges, top_n)` method
- `e0_controller/chess_e0.py` — refactored to use both new methods
- `e0_controller/test_c73_primitives.py` — 18 tests across 4 classes:
  - `TestFullyConnectedConstruction` (7): edge counts, defaults, custom params, empty/single
  - `TestFullyConnectedEquivalence` (1): exact match with manual loop construction
  - `TestStrategyProfileBasic` (7): ranking, quality signs, top_n, sorting
  - `TestStrategyProfileEdgeCases` (3): empty historization, explicit edges, untouched excluded

**Result**  
- `fully_connected(["A","B","C"])` → 3 states, 6 edges, correct Δ/R₀
...
### C75–C81 — Empirical Validation Phase (Explore Scripts + Formal Tests)

**Claim**  
Seven empirical investigations validate E₀ properties through systematic exploration. These commits produced explore scripts with structured experiments and documented findings. Formal pytest test suites now validate the key assertions.
...
**Formal Test Suites**
- `e0_controller/test_attractor_empirical.py` — 24 tests across 4 classes:
  - `TestAttractorUniversality` (5): uniform topology, ratio well-defined, FC+all_success no attractor, FC+differential can create, both conditions needed (C75)
  - `TestAttractorMetrics` (4): concentration range, uniform baseline, state metrics complete, uniformize landscape (C75)
  - `TestMultiAttractorDynamics` (8): cluster structure, shared H dominant, impermeable walls, Gini valid/equal/extreme, asymmetric dominance, weak boundaries (C76)
  - `TestAttractorPrediction` (7): predictors computed, goal-distance valid, goal-distance predicts, PageRank uniform on FC, betweenness uniform on FC, degrees correct, BFS distances (C80)
- `e0_controller/test_learning_empirical.py` — 18 tests across 5 classes:
  - `TestTransferLearningDeterministic` (4): cold reaches goal, transfer doesn’t hurt, inject modifies H, convergence fast (C77)
  - `TestTransferLearningStochastic` (3): corridor valid, learning curve, transfer speedup > 1.0 (C77)
  - `TestConvergenceSpeedDeterministic` (3): stabilizes fast, rank correlation high, stabilization found (C78)
  - `TestConvergenceSpeedRhoSensitivity` (3): t_95 formula, higher ρ → more load, metrics well-formed (C78)
  - `TestAsymmetricRho` (5): settable, speedup > 1.0, failure traces preserved, symmetric default, goal still reachable (C79)

**Status**  
✅ Confirmed — explore scripts validated, formal pytest suites added
...
**Claim**  
M_H as graduated overlap functional (Ontodynamics §3.4) is integrated into the controller's greedy decision loop, not just transition_field(). `_penalized_tension()` divides S_eff by M_H(x,y) when `overlap_modulation=True`. Lower M_H (weak structural support) yields higher effective tension, so the greedy selection prefers well-supported edges. On simple domains without directed triangles (DAGs, trees, cycles), M_H = 1.0 everywhere and behavior is unchanged.

**Evidence**  
- `e0_controller/controller.py` — `_penalized_tension()` extended with overlap modulation
- `e0_controller/test_graduated_overlap.py` — 24 tests across 8 classes:
  - `TestGreedyOverlapIntegration` (6): indistinguishable without overlap, prefers supported with overlap, select_next/run validate, penalty ratio matches M_H
  - `TestNeutralityOnSimpleDomains` (3): linear DAG, Gordian Trap unchanged, identical run paths
  - `TestDualSupport` (2): 2 supports < 1 support in adjusted tension, select_next picks dual-support
  - `TestAsymmetricSupportStrength` (2): strong bypass preferred over weak bypass, run validates
  - `TestOverlapWithHistorization` (2): penalties stack, historization can overcome overlap advantage
  - `TestOverlapModulationFlag` (3): off by default, toggle on activates, M_H > 0 always
  - `TestEscalationWithOverlap` (1): DEAD_END recovery already overlap-aware via transition_field
  - `TestSelfGraphTracking` (1): self_graph records overlap as active component
  - `TestMathematicalProperties` (4): neutral division, amplification correct, ordering preserved, inf unchanged

**Result**  
- Falsification domain validated: S→A (bridge, unsupported) vs S→B (bypass via C) — identical S_eff, controller selects B
- Neutral on 35+ simple domains (DAGs, trees, cycles without bypass)
- M_H > 0 always (Canon §3.4: stability requires non-zero overlap)
- Closes M_H research question (docs/research/E0_MH_ADJUDICATION_RESEARCH_NOTE_v1.md)
...
### C100 — Modulation Benchmark (14 domains × 3 modes)

**Claim**  
The greedy formula `argmin S_eff/(M_H·I)` with overlap modulation (C98) and inertia dampening (C99) preserves domain-invariance across all 10 standard domains (C53) while demonstrably improving navigation in 4 dedicated stress domains. Modulation never costs extra steps on any domain.

**Evidence**  
- `e0_controller/benchmark_modulation.py` — 14 domains × 3 modes (BASELINE, OVERLAP, FULL)
- `e0_controller/test_benchmark_modulation.py` — 29 tests across 8 classes:
  - `TestStressDomainIntegrity` (4): D11-D14 topologies well-formed, goals reachable
  - `TestAllDomainsReachGoal` (3): all 14 domains reach goal in BASELINE, OVERLAP, FULL
  - `TestDomainInvariance` (5): 10 standard domains unchanged by modulation, no extra steps
  - `TestOverlapEffect` (4): D6 saves steps, D12 path changes with overlap
  - `TestInertiaEffect` (4): D11 inertia flips confused→clean, D13 inertia routes around confused path
  - `TestModulationNeutrality` (3): neutral on fresh domains, overlap+inertia compose
  - `TestModulationComposition` (3): FULL includes all OVERLAP changes, never worse than either alone
  - `TestBenchmarkInfrastructure` (3): ModulationComparison dataclass, JSON serialization, pretty-print

**Result**  
- All 14 goals reached in all 3 modes (domain-invariance preserved)
- D6: overlap saves 3 steps (avoids failing G2 path)
- D11: inertia flips choice (confused edge with lower R₀ avoided)
- D12: overlap prefers bypass-supported path (triangle structure)
- D13: inertia routes around confused direct path
- 10 standard domains: zero path changes (neutrality confirmed)
...
### C101 — Scoped Reflexion (Historization-Driven Locality)

**Claim**  
As historization accumulates, reflexion should become local. Global reflexion is the degenerate case of a fresh system. Accumulated trace_load defines a natural scope via `locality = mean_load / (mean_load + μ)`. Scope radius decreases monotonically with inscription depth: fresh systems get global proposals (radius = diameter), historized systems get local proposals (radius → 1). Scoped reflexion restricts both candidate selection and pattern extraction to the scope boundary. Fresh landscape: scoped reflexion ≡ global (no regression). Historized: fewer, locally-informed proposals.

**Evidence**  
- `e0_controller/scoped_reflexion.py` — ReflexionScope, compute_reflexion_scope, scoped_propose_edges, run_with_scoped_reflexion
- `e0_controller/test_scoped_reflexion.py` — 35 tests across 8 classes:
  - `TestScopeComputation` (7): fresh=global, historized=local, monotonic, center included, radius≥1, μ sensitivity, empty
  - `TestBfsNeighborhood` (4): radius 0/1/full, grid neighborhood
  - `TestGraphDiameter` (3): chain=5, single=1, fully-connected=1
  - `TestScopedPattern` (3): local edges only, global scope=global pattern, no-experience fallback
  - `TestScopedProposals` (5): within scope only, fresh=global, historized=local, rationale, no candidates
  - `TestFreshDegeneracy` (2): chain+grid fresh same results as global
  - `TestHistorizedLocality` (3): inscription narrows scope, fewer proposals, local pattern differs
  - `TestRunner` (4): reaches goal, diamond topology, records scopes, inscription narrows
  - `TestMathematicalProperties` (4): locality ∈ [0,1), radius ∈ [1,D], scope ⊆ landscape, dataclass consistency

**Result**  
- Fresh landscape: locality=0, radius=diameter, scope=all states — equivalent to global reflexion
- Historized landscape: locality>0.5 (at μ=1), radius<diameter, proposals confined to local region
- Locality monotonically increases with inscription
- Scoped pattern extraction uses only local edges (different from global average)
- Runner reaches goal on chain and diamond topologies
...
### C102 — Scoped Reflexion Controller Integration

**Claim**  
C101 scoped reflexion can be wired into the integrated reflexion pipeline (C59) via a `scoped=True` parameter, making historization-driven locality available to the full navigation pipeline without breaking backward compatibility. When `scoped=False` (default), behavior is identical to pre-C102. When `scoped=True`, `integrated_reflexion()` and `run_with_integrated_reflexion()` delegate topology proposals to `scoped_propose_edges()` instead of global `propose_edges()`. Scope metadata (ReflexionScope) is accumulated in the result.

**Evidence**  
- `e0_controller/integrated_reflexion.py` — extended with `scoped`, `scope_mu` parameters; imports from `scoped_reflexion`
- `e0_controller/test_integrated_scoped_reflexion.py` — 18 tests across 5 classes:
  - `TestIntegratedReflexionScoped` (6): scoped proposals at frontier, scope metadata populates, empty when unscoped, no frontier = no scope, combined with flags, mu sensitivity
  - `TestBackwardCompatibility` (2): default is unscoped, result.scopes always exists
  - `TestRunnerScoped` (5): runner reaches goal, collects scopes, unscoped empty, connected no scopes, mu passed through
  - `TestFreshVsHistorized` (2): fresh = wide scope, heavy historization = narrow scope
  - `TestEdgeCases` (3): enable_topology=False suppresses scopes, single-state graceful, default empty scopes

**Result**  
- `scoped=False` (default): behavior identical to pre-C102 — no regression
- `scoped=True`: proposals via scoped_propose_edges, scope metadata in result.scopes
- IntegratedReflexionResult.scopes: List[ReflexionScope] — empty by default, populated when scoped
- Runner accumulates scopes across multiple frontier encounters
- scope_mu parameter controls locality sensitivity (smaller = more local)
...
### C103 — Scoped Reflexion Benchmark (10 Domains × GLOBAL vs SCOPED)

**Claim**  
Historization-scoped reflexion (C101) maintains full goal reach parity with global proactive reflexion (C57) across all 10 standard benchmark domains. On fresh landscapes, scoped degenerates to global (locality ≈ 0). On frontier domains, scoped produces equal or fewer proposals with identical effectiveness. Scoped reflexion is a strict non-regression over global.

**Evidence**  
- `e0_controller/benchmark_scoped_reflexion.py` — ScopedResult, ScopedComparison, ScopedBenchmarkResult, run_scoped_benchmark
- `e0_controller/test_benchmark_scoped_reflexion.py` — 22 tests across 6 classes:
  - `TestGoalReachInvariance` (4): global 10/10, scoped 10/10, equal reach, per-domain check
  - `TestNoRegression` (3): scoped never loses goal, ≤ proposals, steps bounded ±2
  - `TestFreshDegeneration` (3): connected domains 0 proposals both modes, identical ratings, identical steps
  - `TestFrontierDomains` (3): D6/D10 trigger proposals, scoped reports locality, fresh frontier low locality
  - `TestBenchmarkStructure` (6): 10 comparisons, types, modes, domain names, efficiency range, global locality = -1
  - `TestSummary` (3): header, all domains, goal counts

**Result**  
- GLOBAL=10/10, SCOPED=10/10 — full goal reach parity
- Equal-or-fewer proposals: 10/10 — scoped never inflates
- Connected domains (D1-D5, D7-D9): 0 proposals both modes, identical steps/ratings
- Frontier domains (D6, D10): proposals fired, locality ≈ 0.04-0.05 (fresh = global degeneration)
- Steps delta: 0 on all domains (no overhead)
...
**Claim**  
Locality is an emergent property of historization, not an imposed parameter. The formula chain `inscription → trace_load → mean_load → locality → radius → scope` produces a continuous transition from global to local as the system ages. Fresh systems (mean_load ≈ 0) degenerate to global reflexion; historized systems (mean_load >> μ) confine reflexion to immediate neighborhoods. This contraction is monotonic, convergent (bounded by ρ-decay equilibrium), and exhibits a phase transition at mean_load = μ (locality = 0.5). Non-uniform inscription creates regional differentiation: heavily inscribed regions have higher local locality than the system average.

**Evidence**  
- `e0_controller/emergent_locality.py` — LocalitySnapshot, LocalityEvolution, RegionalProfile, track_inscription_locality, track_locality_evolution, compute_regional_profile, find_phase_transition, theoretical_phase_transition, theoretical_equilibrium_nonuniform, convergence_rate_bound, track_nonuniform_convergence
- `e0_controller/test_emergent_locality.py` — 50 tests across 14 classes:
  - `TestLocalityMonotonic` (3): chain, star, increases from zero
  - `TestRadiusContraction` (3): monotonic, initial=diameter, decreases
  - `TestPhaseTransition` (4): exists small μ, find_phase_transition, large μ delays, at 0.5
  - `TestConvergence` (3): bounded <1, stabilizes, equilibrium depends on ρ
  - `TestFreshDegeneration` (4): locality=0, radius=diameter, scope=all, mean_load=0
  - `TestRegionalProfile` (4): uniform low diff, non-uniform creates diff, covers all, hot detection
  - `TestNavigationTracking` (4): increases during navigation, monotonic, snapshots, step numbers
  - `TestTheoreticalPrediction` (5): small μ, large μ, impossible (μ·(1-ρ)≥1), ρ=1 edge case, order of magnitude
  - `TestEdgeCases` (5): empty, single edge, μ sensitivity, coverage, summary
  - `TestTheoreticalEquilibriumNonuniform` (4): uniform matches §5.3, single edge, zero edges, more edges lower (C108)
  - `TestConvergenceRate` (3): geometric decay, same rate different topologies, bound approaches zero (C108)
  - `TestTopologyDependentEquilibrium` (3): chain=star (same |E|), sparse>dense, grid between chain and complete (C108)
  - `TestNonuniformConvergenceEmpirical` (5): chain converges, complete converges, chain>complete ordering, gap bounded, multi-edge faster (C108)
  - Helpers: `_make_complete(n)`, `_make_grid(rows, cols)`

**Result**  
- Locality monotonically increases under inscription (both uniform and navigation)
- Radius monotonically decreases (scope contracts)
- Phase transition at round 2 (chain, μ=1.0): locality crosses 0.5
- Convergence: locality saturates at ~0.714 (chain, μ=1.0, ρ=0.9)
- Non-uniform inscription: inscribed regions show higher local_locality than global
- Theoretical prediction within 5× of observed (simple geometric series vs K2 decay)
- Fresh degeneration: locality=0, radius=diameter, scope=all states
...
### C108 — Asymptotic Tightness (Non-Uniform Convergence)

**Claim**  
Under non-uniform inscription (only $k$ of $|E|$ edges inscribed per round), the locality parameter converges to $\ell^* = k / (k + |E| \cdot \mu \cdot (1-\rho))$ with convergence rate $O(\rho^n)$. The rate $\rho^n$ is topology-independent (the system forgets its initial condition at the same geometric rate for any graph). The equilibrium $\ell^*$ is topology-dependent: sparser graphs (smaller $|E|$) achieve higher equilibrium for equal inscription budget. When $k = |E|$ (uniform), the formula reduces to §5.3's result. This resolves P5 §10.4 Q4.

**Evidence**  
- `e0_controller/emergent_locality.py` — `theoretical_equilibrium_nonuniform(k, edge_count, mu, rho)`: returns $\ell^*$. `convergence_rate_bound(rho, n, m_star, mu)`: upper bound on gap. `track_nonuniform_convergence(landscape, current, inscribed_edges, rounds, goal, mu)`: empirical non-uniform inscription driver.
- `e0_controller/test_emergent_locality.py` — 15 tests in 4 classes:
  - `TestTheoreticalEquilibriumNonuniform` (4): uniform matches §5.3, single edge, zero edges, more edges lower
  - `TestConvergenceRate` (3): geometric decay, same rate different topologies, bound approaches zero
  - `TestTopologyDependentEquilibrium` (3): chain=star (same |E|), sparse>dense, grid between chain and complete
  - `TestNonuniformConvergenceEmpirical` (5): chain converges, complete converges, chain>complete ordering, gap bounded, multi-edge faster

**Result**  
- Theoretical formula matches §5.3 when $k = |E|$ (uniform case)
- Convergence rate bound decays geometrically: $\text{bound}(n+1) / \text{bound}(n) \leq \rho$
- Same $\rho$ → same rate for chain and complete graph (topology-independent rate)
- Topology ordering: chain ($|E|=n-1$) > grid ($|E|\approx 2n$) > complete ($|E|=n(n-1)/2$)
- Empirical gap bounded by theoretical bound (with small discretization tolerance)
...
### C109 — Dream Mode Phase 1 (Edge Fingerprints + Functional Equivalence)

**Claim**  
Edges in different domains that play the same navigational role (similar trace_quality, trace_load, inertia_factor) can be detected as functionally equivalent through fingerprint comparison. The fingerprint distance metric is a valid metric (symmetry, triangle inequality, non-negative). Dream readiness — mean inertia_factor across all edges — indicates when a domain's fingerprints are stable enough for cross-domain comparison. Detected equivalences form a Dream Landscape that is itself a valid E₀ Landscape.

**Evidence**  
- `e0_controller/dream_mode.py` — `edge_fingerprint()`, `domain_fingerprints()`, `fingerprint_distance()`, `find_equivalences()`, `dream_readiness()`, `is_dream_ready()`, `build_dream_landscape()`
- `e0_controller/test_dream_mode.py` — 32 tests in 7 classes:
  - `TestEdgeFingerprint` (5): fresh fingerprint, SUCCESS inscription, FAILURE inscription, count, immutability
  - `TestFingerprintDistance` (6): identity, symmetry, triangle inequality, quality dominance, sigmoid normalization, non-negativity
  - `TestEquivalenceDetection` (7): empty domain, fresh domains, sorting, quantile filter, max_results, confidence, properties
  - `TestDreamReadiness` (5): fresh=high, empty=zero, mixed inscription, pure success, threshold
  - `TestDreamLandscape` (6): empty, bidirectional, state encoding, resistance scaling, valid E₀, shared states
  - `TestP1FunctionalEquivalence` (2): same pattern detected, different domains same inscription (P1)
  - `TestP5DomainOrder` (1): order invariance (P5)

**Result**  
- Fingerprint distance is a valid Euclidean metric in normalized (q, m/(m+μ), I) space
- SUCCESS edges in structurally different domains pair with SUCCESS edges (not with FAILURE edges)
- Quantile-based threshold adapts to domain-pair similarity (no fixed ε)
- Dream Landscape is valid E₀ Landscape: can historize, navigate, converge
- Equivalence detection is order-invariant: find_equivalences(A,B) = find_equivalences(B,A)

**Status**  
✅ Confirmed

### C110 — Dream Mode Phase 2 (DreamObserver + Dream Cycle)

**Claim**  
A `DreamObserver` can hold multiple domain Landscapes, run iterative dream cycles that passively compare fingerprints across domain pairs, and incrementally build a Dream Landscape. Feedback (outcome historization) on dream edges allows the system to strengthen valid cross-domain analogies and suppress noise. Querying equivalences by domain returns only edges above a quality threshold, sorted by trace_quality descending (P4: historization filters noise).

**Evidence**  
- `e0_controller/dream_mode.py` — `DreamObserver` class: `register()`, `unregister()`, `dream_cycle()`, `feedback()`, `equivalences_for()`, `readiness_report()`, `summary()`; `DreamCycleResult` dataclass
- `e0_controller/test_dream_mode.py` — 26 additional tests (58 total) in 5 classes:
  - `TestDreamObserverBasics` (7): register, unregister, missing key, initial state, readiness report, summary
  - `TestDreamCycle` (8): two fresh domains, skips unready, three domains, incremental no duplicates, incremental new domain, valid landscape, empty observer, single domain no pairs
  - `TestDreamFeedback` (5): success historization, failure historization, unknown edge ignored, feedback before cycle, both directions updated
  - `TestDreamQuery` (4): equivalences for domain, unknown domain, min quality filter, sorted by quality
  - `TestP4NoiseFiltering` (3): bad analogies suppressed, good analogies strengthen, min quality filters noise

**Result**  
- Dream cycles partition domains by readiness; unready domains are skipped
- Incremental updates add new equivalences without duplicating known edges
- Feedback historizes bidirectionally (source→target and target→source)
- `equivalences_for()` returns results filtered by `min_quality` and sorted by `trace_quality` desc
- P4 confirmed: repeated FAILURE feedback suppresses bad analogies; repeated SUCCESS strengthens good ones

**Status**  
✅ Confirmed

### C111 — Dream Mode Phase 3 (Bridge Hypothesis Generation)

**Claim**  
Dream equivalences can be converted into concrete cross-reflexion proposals via `propose_bridges()`. The coupling discount is derived from dream equivalence trace_quality — high-quality equivalences get more trust, negative-quality ones are excluded. The `make_dream_peer_fn()` integrates dream bridges into E0Controller's peer_fn interface without controller changes. P2: bridge proposals add edges to stuck target domains. P3: FAILURE feedback on dream edges self-corrects, reducing or eliminating future bridges from bad partners.

**Evidence**  
- `e0_controller/dream_mode.py` — `BridgeHypothesis`, `DreamBridgeResult`, `dream_coupling_discount()`, `propose_bridges()`, `make_dream_peer_fn()`
- `e0_controller/test_dream_mode.py` — 21 additional tests (79 total) in 5 classes:
  - `TestDreamCouplingDiscount` (5): full quality, half quality, zero, negative, custom base
  - `TestProposeBridges` (7): basic proposal, unknown domain, no dream landscape, cross-reflexion integration, max_bridges limits, quality modulates discount, negative quality skipped
  - `TestP2Acceleration` (2): bridge adds edges, bridge enables goal reach
  - `TestP3SelfCorrection` (2): failure feedback reduces discount, full self-correction path
  - `TestMakeDreamPeerFn` (5): callable, no dream landscape, returns neighbor, ignores non-neighbor, E0Controller integration

**Result**  
- `dream_coupling_discount(q, base) = base × max(0, q)` — linear scaling, negative quality → 0
- `propose_bridges()` groups equivalences by partner domain, consults best partner first
- Cross-reflexion proposals carry `donor_name="dream:{partner}"` for traceability
- P2 confirmed: bridge proposals add edges to target domain that was at frontier
- P3 confirmed: FAILURE feedback → negative trace_quality → discount=0 → no bridges
- `make_dream_peer_fn()` returns peer_fn compatible with E0Controller — non-invasive integration

**Status**  
✅ Confirmed

### C112 — Dream Mode Phase 4 (End-to-End Exploration)

**Claim**  
The full Dream Mode pipeline (inscription → dream cycles → bridge proposals → self-correction → peer integration) works end-to-end across 6 structurally diverse domains. The Dream Landscape converges (no new equivalences after cycle 1). Bridge proposals add edges to stuck targets. FAILURE feedback eliminates bad bridges (P3). Dream peer_fn integrates with E0Controller without changes.

**Evidence**  
- `e0_controller/explore_dream_mode.py` — 6 phases: inscription (15 runs × 6 domains), dream cycles (5 passes), equivalence precision analysis, bridge experiment (D3_gordian as target), self-correction (P3 via D4_greedy), peer integration (D2_diamond with dream peer_fn)
- Empirical results:
  - 211 equivalences in cycle 1, 0 new in cycles 2–5 (converged)
  - 69 Dream Landscape states, 422 edges, 15 domain pairs
  - D5_grid acts as universal hub (most cross-domain connections)
  - 3 bridge proposals added to D3_gordian
  - P3: bridges drop 3→0 after FAILURE feedback
  - D2 with dream peer_fn: S→A→G in 2 steps, goal reached

**Result**  
- Dream Landscape convergence confirmed (all new equivalences found in first cycle)
- Bridge proposals work end-to-end (dream equivalences → cross_propose_edges → edges added)
- P3 self-correction confirmed empirically (not just unit tests)
- Peer integration seamless (E0Controller unaware of dream mechanism)
...
### C122 — Ontodynamics Canon v2.0

**Claim**  
The ontodynamics JSON canon can be rebuilt from v1.2 (19 nodes, 31 edges, German IDs) to v2.0 (51 nodes, 93 edges, English IDs) with a two-layer architecture (canonical levels 0-8, implementation levels 9-17) while maintaining all existing tests. The canon loader, bootstrapper, and self-bridge pipeline must adapt without code changes — only JSON + test threshold updates.

**Evidence**  
- `e0_controller/canons/ontodynamics.json` — v2.0: 51 nodes, 93 edges, 18 derivation levels
- `e0_controller/canons/ontodynamics_v1.json` — archived v1.2 for comparison
- `e0_controller/test_canon_loader.py` — updated node/edge counts (51/93)
- `e0_controller/test_canon_self_bridge.py` — updated for English IDs
- `e0_controller/test_honest_self_knowledge.py` — updated for v2 coverage
- `e0_controller/test_system_integration.py` — all German IDs → English
- `e0_controller/demo_canon_exposition.py` — all German IDs → English

**Result**  
- Canon loads correctly with 51 nodes, 93 edges
- Canon self-bridge CANON_PROCESS_MAP expanded (33/51 = 65% coverage)
- All navigation code uses English IDs
- Pipeline (JSON → canon_loader → bootstrapper → Landscape) works unchanged
...
### C122b — German ID Cleanup in Navigation Code

**Claim**  
All `iterate()` calls using German state IDs (`differenz`, `negative_notwendigkeit`) were silently navigating via Dead-End escalation rather than canonical derivation. Switching to English IDs (`difference`, `negative_necessity`) restores intended navigation paths.

**Evidence**  
- `e0_controller/test_system_integration.py` — 10 `iterate()` calls updated
- `e0_controller/test_canon_loader.py` — 1 navigation call updated
- `e0_controller/demo_canon_exposition.py` — navigation + LLM prompts updated

**Result**  
- Navigation follows canonical derivation paths instead of Dead-End recovery
- All 3464 tests pass
...
### C122c — Dead-End Sink Repair

**Claim**  
7 implementation nodes had 0 outgoing edges, creating topological sinks that caused DEAD_END escalation during navigation. Adding feedback edges to close these sinks should restore navigability while preserving the only legitimate sinks (goal states).

**Evidence**  
- `e0_controller/canons/ontodynamics.json` — 7 feedback edges added (edge count 86 → 93)
- Feedback edges: cross_reflexion→historization, born_sampling→greedy_navigation, novelty_gate→cross_reflexion, asymmetric_coupling→cross_reflexion, inscription_threshold→historization, observation→reflexivity, spacetime→historization

**Result**  
- Only 2 sinks remain: `negative_necessity` and `sleep_wake_cycle` (both goal states)
- cross_reflexion no longer causes DEAD_END escalation
- All 3464 tests pass
...
**Claim**  
50/93 edges had s_eff = 0 due to initial_U=8-10 with initial_F=0-1, producing δ_H ≈ −1.5 that drove R_eff to floor. These edges were epistemically dead — greedy always selected them, no historization occurred, no learning was possible. Uniform U=2, F=1 should restore epistemic liveness while maintaining correct decay direction (F/U < 0.75).

**Evidence**  
- `e0_controller/canons/ontodynamics.json` — all 93 edges set to initial_U=2, initial_F=1
- `e0_controller/test_canon_loader.py` — trace quality threshold updated (0.5 → 0.0)
- `e0_controller/test_system_integration.py` — reflexive action budget widened (20/4 → 40/8)

**Key findings (empirically verified):**
- Greedy under pure SUCCESS **never** escapes the reflexion loop (10,000 steps, trap_seff remains 0.000)
- F/U threshold at λ_s/λ_f = 0.75 determines decay direction: below = amplify (correct), above = heal (incorrect)
- Born Sampling: 28% success rate (topology-dependent, not U/F-dependent)
- After fix: all edges s_eff > 0 (min: 0.030), correct decay direction, reflexive action gate operational

**Result**  
- Zero epistemically dead edges
- Every traversal produces meaningful historization data
- Reflexive action fires correctly with pre-poisoned self-graph
- 3464 tests pass
...
### C123 — Curriculum Navigator (hierarchical learning for canonical landscapes)

**Claim**  
A 51-node canonical landscape cannot be meaningfully learned in a single run (greedy covers ~12% of edges). Curriculum learning solves this by: (1) splitting the derivation hierarchy into cumulative turns, (2) transferring historization across turns, and (3) replacing goal-reaching with equilibrium detection (T_s stability) as the stopping condition.

**Evidence**  
- `test_curriculum.py` — 35 tests across 6 test classes:
  - `TestEquilibriumDetector` (6 tests): threshold, patience, spike reset, observations
  - `TestCurriculumStrategy` (7 tests): auto-boundaries, custom boundaries, cumulative turns, goal selection within scope
  - `TestScopedLandscape` (6 tests): node/edge filtering, initial U/F preservation, cross-scope exclusion
  - `TestHistorizationTransfer` (3 tests): shared edge transfer, missing edge skip, new edge preservation
  - `TestCurriculumRunner` (9 tests): end-to-end on ontodynamics, final landscape, summary, custom strategy, equilibrium detection, historization carry-across, T_s tracking
  - `TestOntodynamicsCurriculum` (4 tests): default strategy generates 3 turns, goal per turn, scoped landscape coverage, coverage increases

**Result**  
- Default strategy for ontodynamics: 3 turns (levels 0–5, 0–11, 0–17)
- Equilibrium detected when T_s stays below threshold for `patience` consecutive episodes
- Historization transfers correctly between turns (shared edges carry traces, new edges keep bootstrap values)
- Goal selection picks highest-level goal_state within each turn's scope
- Start selection uses lowest derivation level node in scope
- 3499 tests pass
...
### C136 — Feature Engineering (9-dim Round-0 Features)

**Claim**  
Richer Round-0 features improve WL node differentiation. Expanding from 4 to 9 features adds: min_q, max_q, median_q (quality extremes), trace_load_mean, trace_load_std (independent volume dimension). trace_load (U+F) is independent from quality (U−F)/(U+F+ε) — two edges with identical quality can have vastly different trace loads.

**Evidence**  
- `dream_mode.py` — `_edge_quality_stats()` expanded: returns 9 floats (was 4). Feature vectors: D0=9, D1=27, D2=81 (was 4/12/36)
- `explore_c136_feature_engineering.py` — Same setup as C135, only Round-0 features changed:

| Depth | C135 (4-dim) | C136 (9-dim) | Δ |
|---|---|---|---|
| D0 | 11/44 | 15/44 | +4 |
| D1 | 31/44 | 29/44 | −2 |
| D2 | 34/44 | 34/44 | +1* |

*C136 run scored 34 (vs C135's 33) due to LLM scoring variance across runs.

- **Critical diagnostic:** 6/10 unmatched nodes had correct pair CLOSER in WL distance but were blocked by mutual-best constraint. 4/10 had genuine semantic confusion (see↔eye, take↔go).

**Result**  
- Marginal improvement (+1), but diagnostic reveals the real bottleneck: **matching algorithm, not features**
- trace_load adds measurable signal at D0 (+4) but 2-hop propagation already captures enough context

**Status**  
✅ Confirmed (marginal). Key diagnostic: problem shifted from "feature quality" to "matching strategy."

---

### C137 — Hungarian Optimal Assignment (BREAKTHROUGH)

**Claim**  
Greedy mutual-best matching leaves correct pairs unmatched when a third node "steals" a partner. Replacing it with the Hungarian algorithm (globally optimal 1:1 assignment minimizing total WL distance) resolves all assignment conflicts.

**Evidence**  
- `dream_mode.py` — New function: `find_wl_node_equivalences_hungarian()` using `scipy.optimize.linear_sum_assignment` on the full 44×44 WL distance matrix
- `explore_c137_hungarian.py` — Controlled comparison, same topology + scoring:

| Method | Correct/44 | Wrong | Precision | Key |
|---|---|---|---|---|
| D (mutual-best, C136) | 34 | 0 | 100% | 10 unmatched |
| E0 (Hungarian d=0) | 21 | 23 | 48% | 9-dim only |
| E1 (Hungarian d=1) | 39 | 5 | 89% | 1-hop |
| **E2 (Hungarian d=2)** | **44** | **0** | **100%** | **BREAKTHROUGH** |

- Hungarian improvement vs mutual-best: D0 +6, D1 +10, D2 +10
- All 6 "mutual-best blocked" nodes recovered
- All 4 "genuine confusions" (see/eye, take/go) also resolved — global optimization avoids cascading errors
- **Matches position-oracle (C133) at 44/44 without using position knowledge**

**Result**  
- **44/44 (100%) correct, 0 wrong, 100% precision**
- Seedless, no LLM in matching, no position oracle
- Purely structural cross-language identification via WL fingerprints + optimal assignment

**Status**  
✅ **BREAKTHROUGH confirmed.** Complete pipeline: LLM teaches monolingually (score 0–10) → Bootstrapper injects traces → WL depth-2 builds 81-dim fingerprints → Hungarian finds globally optimal 1:1 assignment. Zero-shot cross-domain translation solved.
...
### C139 — DreamObserver + Hungarian/WL Node Equivalences (Runtime Integration)

**Claim:** The proven Hungarian+WL node-level equivalence algorithm (C137, 100% accuracy) can be wired into the DreamObserver.dream_cycle() runtime alongside existing edge-fingerprint equivalences, producing node-level matches in the Dream Landscape without breaking backward compatibility.

**Modules under test:**
- `e0_controller/dream_mode.py` — Extended: `DreamObserver.__init__` (new `node_equivalence_method`, `wl_depth` params), `dream_cycle()` (node-EQ step after edge-EQ), `_update_dream_landscape_nodes()`, `_node_equivalence_state()`, `DreamCycleResult` (+`node_equivalences_found`, +`node_equivalences_new`)
- `e0_controller/test_dream_mode.py` — 10 new tests in `TestNodeEquivalenceIntegration`

**Tests:**
| Test | What it validates |
|------|-------------------|
| `test_invalid_method_raises` | ValueError for unknown `node_equivalence_method` |
| `test_none_method_backward_compatible` | `None` → no node-EQs, edge-EQs still work |
| `test_hungarian_finds_node_equivalences` | `"hungarian"` method finds node matches between isomorphic domains |
| `test_wl_method_finds_node_equivalences` | `"wl"` (mutual-best) also finds node matches |
| `test_dream_landscape_contains_node_states` | Dream Landscape populated with `"domain:node"` states (no `→`) |
| `test_node_equivalence_state_helper` | `_node_equivalence_state()` encodes `NodeFingerprint` as `"domain:node"` |
| `test_incremental_no_duplicate_nodes` | Second cycle re-detects but adds 0 new node-EQs |
| `test_single_domain_no_node_equivalences` | Single domain → no pairs → 0 node-EQs |
| `test_three_domains_node_equivalences` | 3 domains → 3 pairs → node-EQs from all |
| `test_edge_and_node_coexist` | Edge states (`→`) and node states coexist in same Dream Landscape |

**Key design decisions:**
- Layered: both edge-EQ and node-EQ run (not replacement)
- `node_equivalence_method=None` is default → fully backward compatible
- Node states use `"domain:node"` encoding (vs edge: `"domain:src→tgt"`)
- Same `_known_edges` set deduplicates both edge and node equivalences

**Result**
- 10/10 tests pass
- 3540 total (was 3530), 0 failures

**Status**
✅ Confirmed. Hungarian+WL wired into runtime. Integration Story Priority 1 complete.
...
### C148 — Parameter Registry: Central E0Config

**Claim:** All hardcoded numerical defaults across 15 production modules can be centralized in a single frozen dataclass (`E0Config`) without changing any behavior. The registry provides absolute transparency about which parameters govern the system, their default values, and a `summary()` method showing deviations from defaults.

**Modules under test:**
- `e0_controller/config.py` — `E0Config` frozen dataclass, `DEFAULTS` singleton, 45+ fields grouped by subsystem
- `e0_controller/test_config.py` — 28 tests across 6 classes

**Wired production modules (import `DEFAULTS`, defaults sourced from config):**
- `controller.py` — alpha, recent_k, max_escalation_R, s_max, c_min, hybrid_horizon, confidence_threshold, overload_threshold
- `historization.py` — rho, lambda_s, lambda_f, delta_max
- `mode_controller.py` — mu, learn_ratio
- `self_graph.py` — sg_core_delta, sg_core_R0, sg_modulation_delta, sg_modulation_R0, sg_rho
- `dual_reflection.py` — sg_load_min, sg_quality_confused, sg_quality_harmful, sg_inertia_warn
- `reflection.py` — 14 refl_* thresholds
- `structural_entropy.py` — mu (5×), theta_base (2×)
- `sleep_wake.py` — mu, max_dream_cycles
- `dream_mode.py` — dream_readiness, dream_quantile, mu, dream_alpha, dream_base_resistance, theta_base, wl_depth
- `overlap.py` — overlap_floor
- `integrated_reflexion.py` — max_proposals (2×), scope_mu (2×)
- `cross_reflexion.py` — gamma_min, max_proposals, coupling_discount
- `multiverse.py` — convergence_window, max_steps_per_turn, coupling_delta, coupling_resistance
- `exploration_policy.py` — warmup, convergence_threshold
- `coupling_router.py` — router_base_resistance, router_min_delta, sg_load_min, sg_quality_confused, sg_quality_harmful

**Tests:**
| Test | What it validates |
|------|-------------------|
| `TestSingleton::test_defaults_is_e0config` | DEFAULTS is E0Config instance |
| `TestSingleton::test_defaults_uses_default_values` | DEFAULTS == E0Config() field-by-field |
| `TestImmutability::test_cannot_set_attribute` | Frozen: alpha assignment raises |
| `TestImmutability::test_cannot_set_mu` | Frozen: mu assignment raises |
| `TestCoreDefaults::test_controller_defaults` | 8 controller params at expected values |
| `TestCoreDefaults::test_historization_defaults` | rho/lambda_s/lambda_f/delta_max |
| `TestCoreDefaults::test_mu_default` | mu == 5.0 |
| `TestCoreDefaults::test_mode_controller_defaults` | learn_ratio == 0.8 |
| `TestCoreDefaults::test_self_graph_diagnosis_defaults` | 4 sg_* thresholds |
| `TestCoreDefaults::test_self_graph_topology_defaults` | 5 sg_* edge parameters |
| `TestCoreDefaults::test_multiverse_defaults` | 4 coupling/convergence params |
| `TestCoreDefaults::test_dream_defaults` | 5 dream_* params |
| `TestCoreDefaults::test_sleep_wake_defaults` | max_dream_cycles == 10 |
| `TestCoreDefaults::test_structural_entropy_defaults` | theta_base == 0.5 |
| `TestCoreDefaults::test_overlap_defaults` | overlap_floor == 0.2 |
| `TestCoreDefaults::test_exploration_defaults` | warmup + convergence_threshold |
| `TestCoreDefaults::test_reflexion_defaults` | max_proposals, coupling_discount, gamma_min |
| `TestCoreDefaults::test_reflection_thresholds` | 4 representative refl_* values |
| `TestCoreDefaults::test_router_defaults` | router_base_resistance, router_min_delta |
| `TestCustomConfig::test_override_single_field` | E0Config(alpha=1.5) leaves others untouched |
| `TestCustomConfig::test_override_multiple_fields` | E0Config(mu=10, rho=0.95) |
| `TestSummary::test_all_defaults_summary` | "all defaults" in summary |
| `TestSummary::test_modified_summary` | Modified fields appear in summary |
| `TestWiringSpotChecks::test_historization_uses_config_rho` | Historization().rho == DEFAULTS.rho |
| `TestWiringSpotChecks::test_controller_uses_config_alpha` | E0Controller default alpha == DEFAULTS.alpha |
| `TestWiringSpotChecks::test_exploration_policy_uses_config_warmup` | ExplorationPolicy default warmup |
| `TestWiringSpotChecks::test_dream_observer_uses_config_mu` | DreamObserver default mu |
| `TestWiringSpotChecks::test_multiverse_uses_config_convergence` | MultiverseController default window |

**Result**
- 28/28 tests pass
- 3608 total (3580 existing + 28 new), 0 failures
- Zero behavioral change: all production defaults identical to pre-C148 values
...
### C149 — μ/α Unification: Function-Level Defaults Flow Through DEFAULTS

**Claim:** C148 wired class constructors; C149 completes unification by wiring every remaining standalone function signature that used `mu: float = 5.0` or `alpha: float = 0.5` (inertia dampening) to `DEFAULTS.mu` and `DEFAULTS.inertia_alpha`. The config field `dream_alpha` is renamed to `inertia_alpha` because the dampening α is used in 5 subsystems, not just dream mode.

**Changes:**
- `config.py` — `dream_alpha` renamed to `inertia_alpha` (with updated docstring: "used in inertia_factor across all subsystems")
- `dream_mode.py` — 7 standalone functions: `edge_fingerprint`, `domain_fingerprints`, `fingerprint_distance`, `find_equivalences`, `dream_readiness`, `is_dream_ready` + DreamObserver constructor updated
- `historization.py` — `inertia_factor(alpha, mu)` now defaults from DEFAULTS
- `self_graph.py` — `component_inertia(alpha, mu)` now defaults from DEFAULTS
- `coupling_router.py` — `CouplingSelfGraph.component_inertia(alpha, mu)` now defaults from DEFAULTS
- `emergent_locality.py` — 7 functions + `LocalityEvolution.mu` dataclass field default
- `rendering_adapter.py` — inline `alpha, mu = 0.5, 5.0` → imports from DEFAULTS

**Result**
- 3608/3608 tests pass, 0 failures
- Zero remaining `mu: float = 5.0` in any production function signature
- Zero remaining `alpha: float = 0.5` (inertia) in any production function signature
...
**Claim:** Given a landscape and multiple E0Config variants, the system runs each config independently, attaches a SelfGraph, diagnoses component health, and compares results. `suggest_perturbations` proposes config variants for harmful/confused components. COMPONENT_PARAMS maps Self-Graph node names to tunable E0Config fields.

**File:** `test_parameter_sensitivity.py` — 28 tests

| Test | Validates |
|---|---|
| `TestComponentParams::test_all_components_mapped` | Every ALL_COMPONENTS entry in COMPONENT_PARAMS |
| `TestComponentParams::test_params_exist_in_config` | All param names are real E0Config fields |
| `TestTrialResult::test_quality_score_empty` | Empty qualities → 0.0 |
| `TestTrialResult::test_quality_score_sum` | Sum of component qualities |
| `TestTrialResult::test_health_score_no_diagnosis` | No diagnosis → 0.0 |
| `TestTrialResult::test_health_score_formula` | +1 healthy − 2 harmful − 1 confused |
| `TestTrialResult::test_counts` | healthy/confused/harmful count properties |
| `TestRunTrial::test_default_config` | run_trial returns valid TrialResult |
| `TestRunTrial::test_has_component_qualities` | Component qualities populated after run |
| `TestRunTrial::test_landscape_independence` | Deep-copy: original landscape untouched |
| `TestRunTrial::test_custom_config` | Custom alpha/recent_k propagated |
| `TestRunTrial::test_with_goal` | Goal-directed trial terminates |
| `TestRunTrial::test_mixed_outcomes` | Mixed SUCCESS/FAILURE produces qualities |
| `TestSensitivityAnalysis::test_two_configs` | Two-config comparison returns SensitivityReport |
| `TestSensitivityAnalysis::test_best_trial_highest_quality` | best_trial has max quality_score |
| `TestSensitivityAnalysis::test_summary_string` | summary() contains Trial labels |
| `TestParameterImpact::test_single_config_empty` | Single trial → no impact |
| `TestParameterImpact::test_varying_param_detected` | Differing alpha appears in impact |
| `TestParameterImpact::test_constant_param_absent` | Same rho excluded from impact |
| `TestSuggestPerturbations::test_none_diagnosis` | None → empty |
| `TestSuggestPerturbations::test_all_healthy` | Healthy → empty |
| `TestSuggestPerturbations::test_harmful_historization` | Historization harmful → rho/λ_s/λ_f/δ_max perturbed |
| `TestSuggestPerturbations::test_harmful_transition_field` | Transition_field harmful → α/recent_k/overload perturbed |
| `TestSuggestPerturbations::test_confused_produces_variants` | Confused triggers perturbations |
| `TestSuggestPerturbations::test_no_params_component` | Amplitude (no params) → empty |
| `TestSuggestPerturbations::test_perturbation_values_float` | Float ±20% around default |
| `TestSuggestPerturbations::test_perturbation_values_int` | Integer deltas remain int |
| `TestSuggestPerturbations::test_custom_base_config` | Perturbations relative to custom base |

**Result**
- 28/28 tests pass
- 3667 total (3608 existing + 28 new + 31 growth), 0 failures
...
### C152 — Hybrid Mode in Sensitivity: 3-Regime Comparison

**Claim:** The parameter sensitivity system (C150) can compare transport regimes (GREEDY vs AMPLITUDE_ON_DISAGREE vs BORN_SAMPLING) and SU(2) variants (U(1) vs SU(2) vs geometric) by forwarding `hybrid_mode`, `hybrid_horizon`, and `use_su2` from E0Config to the controller in `run_trial()`. This enables the usage pattern from the SU(2) research doc §5 Phase 2+3.

**Key changes:**
- `E0Config` gains `hybrid_mode: str = "greedy"` and `use_su2: object = False`
- `run_trial()` forwards `hybrid_mode`, `hybrid_horizon`, `use_su2` to E0Controller
- `COMPONENT_PARAMS` updated: amplitude → [hybrid_mode, confidence_threshold, hybrid_horizon]; born → [hybrid_mode, use_su2, hybrid_horizon]
- `suggest_perturbations()` auto-generates numeric perturbations for harmful amplitude/born (horizon, threshold)

**File:** `test_parameter_sensitivity.py` — 13 new tests (41 total in file)

| Test | Validates |
|---|---|
| `TestSuggestPerturbations::test_no_params_component` | Updated: realization (not amplitude) has no params |
| `TestSuggestPerturbations::test_harmful_amplitude_produces_numeric_perturbations` | amplitude harmful → hybrid_horizon perturbations |
| `TestHybridModeForwarding::test_greedy_excludes_amplitude_born` | GREEDY → amplitude/born absent from qualities |
| `TestHybridModeForwarding::test_born_mode_activates_amplitude_and_born` | BORN_SAMPLING → both components active |
| `TestHybridModeForwarding::test_amplitude_on_disagree_activates_amplitude_only` | amplitude active, born not |
| `TestHybridModeForwarding::test_hybrid_horizon_forwarded` | horizon=5 runs without error |
| `TestHybridModeForwarding::test_use_su2_forwarded` | use_su2=True + born → SU(2) transport |
| `TestHybridModeForwarding::test_use_su2_geometric` | use_su2="geometric" runs successfully |
| `TestSensitivityWithModes::test_greedy_vs_born_different_qualities` | Different component profiles |
| `TestSensitivityWithModes::test_parameter_impact_detects_hybrid_mode` | hybrid_mode in impact ranking |
| `TestSensitivityWithModes::test_three_regime_comparison` | U(1)/SU(2)/geometric all have born active |
| `TestComponentParamsC152::test_amplitude_has_params` | hybrid_mode, confidence_threshold, hybrid_horizon |
| `TestComponentParamsC152::test_born_has_params` | hybrid_mode, use_su2, hybrid_horizon |
| `TestComponentParamsC152::test_no_params_components_unchanged` | realization, inertia, curvature, overlap = [] |

**Result**
- 3659/3659 tests pass, 0 failures
- Sensitivity system now supports full transport-regime comparison
- COMPONENT_PARAMS gap closed: amplitude and born have actionable parameters
...
### C153 — SU(2) Perspective Diagnostic

**Claim:** A `perspective_check(controller, current, horizon, geometry, goals)` function compares U(1) vs SU(2) vs geometric rankings for the same state, producing a `PerspectiveReport` with ranking agreement, robust/fragile classification, and fragile action warnings. This is wired into `controller.cycle()`, `controller.run()`, and `DualReflectionReport`.

**Key changes:**
- `perspective_diagnostic.py` — `perspective_check()`, `PerspectiveReport`, `_ranking_agreement()`, `_fragile_actions()`
- `controller.py` — `StepResult.perspective`, `cycle(perspective_horizon=)`, `run(perspective_horizon=)`
- `dual_reflection.py` — `DualReflectionReport.perspective`, `_cross_reference(perspective=)`, format section

**File:** `test_perspective_diagnostic.py` — 35 tests

**Result**
- 3694/3694 tests pass, 0 failures
...
### C154 — DreamObserver Node Bridge Pipeline

**Claim:** Node-level equivalences from Hungarian matching (C137/C139) can be operationalized for structural edge transfer: if node A in domain X is equivalent to node B in domain Y, and Y has outgoing edges from B that X lacks from A, those edges are proposed as bridge hypotheses with discounted resistance. A `confidence_floor` parameter breaks the cold-start chicken-and-egg (fresh equivalences have trace_quality=0 but come from structural matching). The existing `make_dream_peer_fn` falls back to node bridges when edge bridges produce nothing. `SleepWakeCycle.wire_peer_fns()` auto-creates dream peer functions for all registered controllers.

**Key changes:**
- `dream_mode.py` — `node_equivalences_for()` query method on `DreamObserver`, `NodeBridgeProposal`/`NodeBridgeResult` dataclasses, `propose_node_bridges()` structural transfer function, `make_dream_peer_fn()` upgraded with node-bridge fallback, `equivalences_for()` now filters out node states (no `→`)
- `sleep_wake.py` — `SleepWakeCycle.wire_peer_fns()` auto-wires dream peer_fns for all registered controllers

**File:** `test_dream_node_bridges.py` — 32 tests in 5 classes:

| Test Class | Tests | Validates |
|---|---|---|
| `TestNodeEquivalencesFor` | 10 | Query method: domain/node filter, min_quality, sort order, empty cases |
| `TestProposeNodeBridges` | 8 | Structural transfer: asymmetric domains, edge addition, statistics, discount, max_proposals |
| `TestMakeDreamPeerFnNodeBridge` | 3 | Node-bridge fallback in peer_fn, neighbor constraint |
| `TestWirePeerFns` | 4 | SleepWakeCycle auto-wiring: count, callable, custom params |
| `TestNodeBridgePipeline` | 4 | End-to-end: dream_cycle → node_eq → propose → peer_fn → sleep-wake |

**Result**
- 3757/3757 tests pass, 0 failures
- Asymmetric transfer validated: donor shortcut A→C transfers to target domain
- confidence_floor=0.1 enables proposals from fresh structural matches

**Status**
✅ Confirmed

---
### C155 — Auto-Tuning (Closed-Loop Parameter Optimization)

**Claim:** The Self-Graph diagnosis pipeline (C150/C151) can be closed into a fully automatic tuning loop: (1) run baseline trial, (2) diagnose via Self-Graph, (3) suggest perturbations for unhealthy components, (4) evaluate perturbation variants via sensitivity analysis, (5) adopt the best config if improvement exceeds threshold, (6) repeat until healthy or budget exhausted. `apply_config()` updates a live controller in-place (including HybridMode string→enum conversion and landscape historization parameters). `Session.auto_tune()` provides a convenience wrapper with optional adoption.

**Key changes:**
- `parameter_sensitivity.py` — `AutoTuneRound`/`AutoTuneResult` dataclasses, `auto_tune()` closed-loop function, `apply_config()` live controller update
- `session.py` — `Session.auto_tune()` convenience wrapper (builds E0Config from controller state, optionally applies result)

**File:** `test_parameter_sensitivity.py` — 25 new tests (66 total in file) in 4 classes:

| Test Class | Tests | Validates |
|---|---|---|
| `TestAutoTuneResult` | 5 | Dataclass properties: improved, improvement, total_trials, summary |
| `TestAutoTune` | 9 | Closed loop: returns result, initial config preserved, max_rounds, healthy early-stop, goal support, round structure, mixed_fn perturbation, custom perturbation_factor |
| `TestApplyConfig` | 7 | In-place update: alpha, historization params, hybrid_mode enum conversion, overload_threshold, use_su2, landscape reference preserved, controller runs after apply |
| `TestSessionAutoTune` | 4 | Session wrapper: runs, adopt=True applies, adopt=False preserves, goal support |

**Result**
- 3782/3782 tests pass, 0 failures
- Healthy baseline stops after 1 round (no unnecessary perturbation)
- apply_config preserves landscape reference (no reconstruction)
- HybridMode string→enum conversion validated
...
**Claim:** The Curriculum Navigator (C123) and Sleep-Wake Cycle (C121) can be wired so that dream consolidation happens *between* curriculum turns, not just post-hoc. After each turn reaches equilibrium (or exhausts episodes), the turn's scoped landscape is registered with a DreamObserver and consolidated via `dream_cycle()`. This enables cross-turn structural equivalence discovery during learning, not just after. `TurnResult.dream_consolidation` captures the dream results per turn, and the observer accumulates domains across the entire curriculum.

**Key changes:**
- `curriculum.py` — `CurriculumRunner.__init__` accepts optional `observer: DreamObserver` and `consolidation_cycles: int`, `_run_turn()` registers turn landscape and runs dream consolidation, `TurnResult.dream_consolidation` field, `observer` property
- Import of `DreamCycleResult`/`DreamObserver` from `dream_mode`

**File:** `test_curriculum.py` — 14 new tests (49 total in file) in 2 classes:

| Test Class | Tests | Validates |
|---|---|---|
| `TestTurnResultDreamConsolidation` | 2 | Default empty list, explicit dream results |
| `TestCurriculumRunnerObserver` | 12 | No observer → empty, observer property, consolidation results, domain registration, equivalence discovery, cycles=0 skips, default cycles=3, summary/final_landscape/historization transfer with observer, dream landscape growth |

**Result**
- 3796/3796 tests pass, 0 failures
- Turn domains registered as `curriculum_turn_{level_max}`
- Dream consolidation finds equivalences across cumulative scoped landscapes
- Historization transfer still works correctly with observer active
...
**Claim:** Dream equivalence quality signals can dynamically bias CouplingRouter partner selection. `update_weights_from_dream()` queries the DreamObserver for edge-level and node-level equivalences per universe, computes mean trace_quality, and maps it to coupling weight: `weight = max(floor, 1.0 + mean_quality)`. Positive equivalences → higher weight → lower R₀ → cheaper to receive from (trusted donor). Negative equivalences → lower weight → higher R₀ → expensive (unreliable). This closes the last integration gap: Dream → CouplingRouter.

**Key changes:**
- `coupling_router.py` — `DreamWeightUpdate`/`DreamWeightReport` dataclasses, `update_weights_from_dream(router, observer, *, min_equivalences, weight_floor)` function, TYPE_CHECKING import of DreamObserver

**File:** `test_coupling_router.py` — 15 new tests (89 total in file) in 3 classes:

| Test Class | Tests | Validates |
|---|---|---|
| `TestDreamWeightUpdate` | 2 | changed property (true/false) |
| `TestDreamWeightReport` | 3 | changed_count, total_equivalences, summary string |
| `TestUpdateWeightsFromDream` | 10 | No equivalences preserves weights, positive raises weight, negative lowers weight, weight_floor respected, min_equivalences threshold, routing effect (R₀ changes), all domains in report, node equivalences included, 3-universe differential, idempotent without new feedback |

**Result**
- 3811/3811 tests pass, 0 failures
- Weight = max(floor, 1.0 + mean_quality): range [floor..2.0]
- Combines edge-level AND node-level equivalences for weight computation
- Idempotent: calling without new feedback produces same weights
...
### WP-2.1 — G1 development-domain infrastructure

**Verified infrastructure**

- `g1_domains.py` generates four preregistered families (`wall_grid`, `trap_grid_v2`, `decoy_dag`, `nonstationary_parallel`) at exactly N = 100/500/1000.
- Outcome draws are keyed by protocol, generator seed, episode, edge, and edge-attempt index; generator order cannot consume a shared environment RNG stream.
- Development seeds are restricted to 0–9. Preregistered holdout seeds 1000–1029 raise `HoldoutAccessError`.
- `py -3 -m e0_controller.g1_harness` writes only development domain records, environment metadata, and a manifest marked `not_g1_result=true`.

**Evidence**

- `e0_controller/test_g1_domains.py` — 20 tests
- Full development matrix smoke test — 120/120 instances, 30 per family, no duplicate instance IDs, no holdout access

**Status**

✅ Infrastructure confirmed. **No method comparison, causal effect, holdout result, or Gate-G1 claim has been produced.**

---
### WP-2.2 — Fair G1 baseline adapters

**Verified infrastructure**

- Eight preregistered adapters share one episode runner and one FAILURE rule: the attempt consumes one interaction and position remains at the source.
- Q-learning, UCB1-edge, and random-restart-greedy are the only G1-B comparator methods.
- Memoryless greedy, ε-greedy, and uniform random are diagnostic only.
- A* and D* Lite receive the full static topology, are labeled map-informed, and are excluded from the comparator.
- BFS remains a map-oracle test for shortest-path optimality; it is not an added protocol method.
- Global parameters and project-specific operationalizations are frozen in `E0_G1_BASELINE_CONFIGS_v1.json`.

**Evidence**

- `e0_controller/test_g1_baselines.py` — 33 tests
- `docs/E0_G1_BASELINE_AUDIT_v1.md` — legacy-runner audit, information contract, references, deviations, and exclusions
- `py -3 -m e0_controller.g1_baseline_harness` — 960/960 development adapter runs completed (120 domains × 8 methods)
- Harness manifest: `holdout_accessed=false`, `not_g1_result=true`, 0 failed adapter runs

**Status**

✅ Baseline compatibility and fairness contract confirmed. **No ranking, selected winner, holdout result, causal effect, or Gate-G1 claim has been produced.**

---

### WP-2.3 — Five-level E₀ causal ablations

**Verified infrastructure**

- `A_HIST` isolates historization plus revisit without lookahead.
- `B_INCOHERENT`, `C_THETA_ZERO`, `D_U1_PHASE`, and `E_FULL_GEOMETRY` share candidates, bounded `simple` path families, horizon 3, the 100,000-path cap, and one conservative override gate.
- B sums individual path intensities; C adds positive-real path mass; D adds U(1) phase; E calls `lean.structural_geometry.influence_map` and checks its returned paths against B–D.
- All variants share empty-start learning, static topology/weights, own observed outcomes, and FAILURE-as-no-transition semantics. Hidden probabilities, future outcomes, the switch, and oracle cost remain unavailable.
- Global operationalization is frozen in `E0_G1_ABLATION_CONFIGS_v1.json`; the primary simpler control remains deliberately unselected pending WP-2.4 full development data.

**Evidence**

- `e0_controller/test_g1_ablations.py` — 34 tests
- `docs/E0_G1_ABLATION_AUDIT_v1.md` — causal boundary, shared gate/path contract, cap behavior, and interpretation boundary
- `py -3 -m e0_controller.g1_ablation_harness` — 600/600 bounded development compatibility runs (120 domains × 5 variants)
- Compatibility manifest: `holdout_accessed=false`, `not_g1_result=true`, 0 path-cap hits

**Status**

✅ Causal adapter contract and complete development-shape compatibility confirmed. **No full-budget performance comparison, selected control, holdout result, causal effect, or Gate-G1 claim has been produced.**

---

### WP-2.4 — Full development execution infrastructure

**Verified infrastructure**

- The exact matrix contains 120 development instances x 13 methods = 1,560
  independent full-budget replicates.
- Every replicate executes 10 adaptation plus 20 evaluation episodes in a
  fresh worker process and writes an atomic, resumable shard.
- Consolidation creates the six protocol artifacts: manifest, frozen configs,
  raw runs, compressed evaluation episodes, summary, and environment.
- The preregistered simpler-control selection and stratified paired bootstrap
  are deterministic and development-only.
- Holdout seeds are rejected before task creation; reports remain explicitly
  marked as not being Gate-G1 results.
- Episode execution now checks the preregistered 60-second deadline at
  decision boundaries; every replicate additionally runs in a killable child
  process with the preregistered 1,800-second hard limit.
- The full matrix partitions deterministically into 240 GitHub Actions jobs of
  6–7 replicates, with at most 20 four-worker jobs in parallel.
- C327's 196 completed local shards are retained as explicitly aborted
  engineering evidence and cannot be merged into the corrected full bundle.
- Run `30438690300` completed all 1,560 source-bound development records on
  commit `af2ae45`: 183 valid algorithmic negatives, zero infrastructure
  failures, no holdout access, and `A_HIST` selected as the frozen simpler
  control for that implementation.
- C330 removes the diagnosed per-decision field rebuild and global phase-solve
  bottleneck while preserving candidates, paths, scores, decisions, and
  interaction semantics within tested tolerances.
- The remediated workflow installs `.[science]` explicitly and records the
  SciPy version.
- Run `30526724307` completed all 242 Actions jobs, including consolidation,
  on clean source commit `3990b5d`. Its verified bundle contains all 1,560
  development records: 1,557 completed, 3 algorithmic timeouts confined to
  `E_FULL_GEOMETRY` on `wall_grid N=1000`, zero infrastructure failures, zero
  path-cap hits, and no holdout access.
- C331 aggregates all 17,929,640 persisted B-E evaluation decisions and finds
  zero overrides. Its 9,600-decision bounded probe locates the confidence gate
  and inactive gradient-only phase regime as the neutralization mechanisms.

**Evidence**

- `e0_controller/test_g1_development_report.py` — 23 tests
- `e0_controller/test_g1_development_distributed.py` — 9 tests
- `e0_controller/test_g1_timeout_enforcement.py` — 3 tests
- `e0_controller/test_g1_ablations.py` — 34 tests; persistent/cold-field and
  sparse-direct/CG development parity
- `e0_controller/test_g1_mechanism_diagnosis.py` — 6 tests; gate blockers,
  pairwise divergence, evidence aggregation, and holdout rejection
- `lean/structural_geometry/tests/test_acceptance.py` — 40 package acceptance
  tests; dense/CG/direct solver agreement and topology invalidation
- `docs/E0_G1_DEVELOPMENT_RUNNER_v1.md` — execution, artifacts, recovery,
  statistics, and decision boundary
- `docs/E0_G1_DISTRIBUTED_EXECUTION_v1.md` — C327 diagnosis, bounded matrix,
  artifact flow, cost boundary, and publication boundary
- `docs/E0_G1_SCALING_DIAGNOSIS_v1.md` and
  `docs/E0_G1_SCALING_REMEDIATION_v1.md` — C329 root cause and C330 repair
- `docs/E0_G1_MECHANISM_DIAGNOSIS_v1.md` and
  `artifacts/g1/E0-G1-v1/development/c331/mechanism_diagnosis.json` — C331
  Development-only neutralization diagnosis and machine-readable evidence
- Full suite: 6,838 passed, 2 skipped, 210 subtests in 3,349.45 seconds
  wall time; the run included a system/monitoring suspension
- C330 focused verification: 69 G1/report/distribution tests plus 40 lean
  structural-geometry acceptance tests passed; collection reports 6,842 tests.

**Status**

✅ The remediated full development execution is complete and source-bound to
`3990b5d`. The scaling defect is substantially reduced, while G1-A and phase
attribution remain development-negative. C331 shows that no B-E lookahead
decision crossed the override boundary and D/E never left the gradient phase
regime. **No holdout access and no Gate-G1 result exist.**

---

### WP-GATE-0.1 — Override gate policy and calibration design

**Claim**

The existing `confidence_threshold`/`min_confidence` scalar is an uncalibrated
support margin, not a portable probability of override quality. The controller,
Structural Geometry, and G1-v1 implement distinct gate contracts, so future
experiments must version the complete policy rather than select an implicit
global scalar.

**Evidence**

- `docs/E0_OVERRIDE_GATE_POLICY_v1.md` — normative terminology, legacy-policy
  audit, policy interface, compatibility rules, and calibration workflow
- `docs/E0_OVERRIDE_GATE_CALIBRATION_PROTOCOL_v1.json` — machine-readable
  design contract and unresolved experiment-instance fields
- `docs/research/C185_TRAFFIC_VALIDATION_REPORT_v1.md` — local evidence for
  `0.85` over `0.5`, scoped to the reported traffic simulation
- `docs/E0_G1_MECHANISM_DIAGNOSIS_v1.md` — `0.85` produced zero overrides in
  the complete G1-v1 development bundle
- `e0_controller/config.py` and `lean/structural_geometry/overlay.py` —
  divergent legacy defaults and guards

**Status**

✅ Policy and calibration design recorded. Runtime behavior, serialized
snapshots, G1-v1, all defaults, and every holdout remain unchanged. No
calibration has been executed and no replacement threshold has been selected.

---

### WP-GATE-0.2 — Versioned policy infrastructure

**Claim**

The complete override contract can be represented, enforced, and persisted
without changing any legacy controller decision or selecting a new threshold.

**Evidence**

- `e0_controller/override_gate.py` — immutable policy/mode types, three exact
  legacy mappings, validation, serialization, fail-closed measurable guards
- `e0_controller/controller.py` — authoritative policy evaluation with the
  scalar alias retained for legacy-controller tuning
- `e0_controller/envelope.py`, `e0_controller/memory_os.py`, and
  `e0_controller/session.py` — policy-aware new records and old-record restore
- `e0_controller/test_override_gate_policy.py` — 35 tests covering policy
  validation, legacy contracts, dynamic guards, live decisions, scalar
  mutation boundaries, Envelope round trips, and MemOS migration
- Focused compatibility suites: 590 tests across the new policy tests,
  confidence, Envelope, MemOS, Session, parameter-sensitivity, amplitude,
  Gordian, mass-trap, evaluation, reflection-hybrid, and paper-default surfaces

**Status**

✅ Policy infrastructure implemented. Default greedy mode remains unchanged;
implicit scalar callers map to `legacy_controller_v1`; old snapshots without a
policy restore their exact scalar; fixed/calibrated/G1 policies cannot be
silently changed through the scalar alias. No calibration or holdout access.

---


### WP-GATE-0.3 — Concrete calibration preregistration

**Claim**

The threshold-selection experiment can be made outcome-independent by freezing
its population, split manifests, candidate policies, utility, risk and
activation budgets, inference, tie-breaker, and verification boundary before
any new domain is instantiated.

**Evidence**

- `docs/E0_OVERRIDE_GATE_CALIBRATION_INSTANCE_v1.json` — complete frozen
  machine-readable instance based on source commit
  `46313f60a1bf0ed8590ec1e05d8a71d31200481a`
- `docs/E0_OVERRIDE_GATE_CALIBRATION_PREREGISTRATION_v1.md` — rationale,
  causal control, interpretation boundary, and no-result statement
- `e0_controller/override_gate_calibration.py` — non-executing loader and
  semantic validator; it neither imports nor invokes domain generators
- `e0_controller/test_override_gate_calibration.py` — 23 tests covering
  status flags, narrow scope, exact population, disjointness from G1-v1
  holdout, candidate grid, counts, risk, stopping, fallback, verification, and
  drift rejection

**Status**

✅ Instance frozen, not executed. Calibration, verification, and protected
holdout flags are false; no threshold is selected; no default or G1-v1
behavior changes.

---

### WP-GATE-0.4 — Domain-free execution scaffold

**Claim**

The frozen instance can be converted into exact execution units and lifecycle
contracts without constructing a domain, exposing an outcome command, or
opening a protected split.

**Evidence**

- `e0_controller/override_gate_calibration_runner.py` — stable task IDs,
  240-batch matrix, shared/co-executed disabled controls, authorization guards,
  artifact validation, sealed dry run, and planning-only CLI
- `docs/E0_OVERRIDE_GATE_CALIBRATION_RUNNER_v1.md` — execution-unit semantics,
  lifecycle boundary, verified counts and hashes, and remaining implementation
- `e0_controller/test_override_gate_calibration_runner.py` — 31 tests covering
  all 3,600 planned split tasks, no-domain planning, calibration completeness,
  verification/holdout authorization, balanced partitioning, control modes,
  artifact leakage flags, deterministic hashes, atomic CLI output, and absence
  of a run command

**Status**

✅ Preflight implemented and verified: 2,880 calibration tasks, 360
verification tasks, 360 protected-holdout tasks, zero domains, zero outcomes.
The execution commit remains unfrozen and no outcome-producing command exists.

---

### WP-GATE-0.5 — Calibration-only domain and branch engine

**Claim**

Fresh calibration domains and local counterfactual utility can be implemented
without weakening G1-v1 seed protection, enabling protected splits, changing a
candidate, or mutating the closed-loop parent trajectory.

**Evidence**

- `e0_controller/g1_domains.py` — explicit default development namespace and
  separate calibration-only namespace; legacy record path rejects calibration
  mislabelling
- `e0_controller/override_gate_calibration_engine.py` — scoped domain builder,
  exact candidate materialization, E_FULL_GEOMETRY policy adapter, canonical
  state hashes, cloned keyed outcome state, paired rollouts, and instrumented
  parent episode
- `docs/E0_OVERRIDE_GATE_CALIBRATION_ENGINE_v1.md` — namespace, snapshot,
  branching, test-data, no-result, and next-runner boundaries
- `e0_controller/test_override_gate_calibration_engine.py` — 39 tests covering
  all families/scales structurally, rejected noncalibration/protected seeds,
  unchanged generator semantics, exact policy enforcement, gate-only decision
  differences, deterministic branches, parent parity, and path-cap failure

**Status**

✅ Engine implemented. Calibration seeds were used only to construct and
validate domain structure; paired outcomes were generated only on a four-node
synthetic fixture outside every experiment split. No calibration, verification,
or holdout result exists, and no execution CLI exists.

---

### WP-GATE-0.8 — Result freeze and disagreement-funnel repair

**Claim**

The completed calibration is retained as immutable negative evidence, while
future shard records distinguish all observed disagreements from those passing
the common guards and from executed threshold overrides.

**Evidence**

- `docs/E0_OVERRIDE_GATE_CALIBRATION_RESULT_v1.md` — provenance, result,
  limitations, timeout structure, wall-grid mechanism, and next boundary
- `artifacts/override_gate/E0-OVERRIDE-GATE-CAL-INSTANCE-v1/calibration/run_30631675731/`
  — full hash-bound evidence ZIP plus readable manifests and diagnosis
- `e0_controller/override_gate_calibration_pipeline.py` — shard schema v2,
  observed-disagreement count, and ordered count invariant
- `e0_controller/override_gate_calibration_statistics.py` — validation and
  candidate reporting of the full disagreement funnel
- focused validation: 47/47 calibration pipeline/statistics tests passed

**Status**

✅ Result frozen and measurement semantics repaired for future evidence. The
selected scoped policy remains `gate_disabled`; verification and protected
holdout were not accessed. No new calibration execution is authorized.

---

### WP-GATE-0.9 — Development-only scaling and activation pilot

**Claim**

The dominant timeout mechanism is synchronous paired-branch instrumentation,
not demonstrably the closed-loop parent alone. The zero-margin gate is a live
code path and can harm a bounded development wall-grid trajectory.

**Evidence**

- `e0_controller/override_gate_development_pilot.py` — development-seed-only,
  wall-grid-only, small-budget, killable parent/paired pilot
- `artifacts/override_gate/development/wp_gate_0_9/` — budget-8 and budget-40
  raw records plus hash-bound diagnosis
- `docs/E0_OVERRIDE_GATE_DEVELOPMENT_PILOT_v1.md` — scaling ratios, timeout
  confound, activation outcome, and required v2 stage separation
- focused validation: 47/47 pilot and branch-engine tests passed

**Status**

✅ Root cause isolated. At budget 40, paired evidence expands 21 parent
geometry decisions to 518 at `N=100` and 40 to 1,356 at `N=500`; `N=1000`
times out only with paired collection. Nine zero-margin overrides turn the
bounded development parent from goal-reaching into budget exhaustion. No
calibration, verification, or protected-holdout execution occurred.

---

### WP-GATE-0.10 — Development-only v2 stage separation

**Claim**

Paired decision evidence can be collected under its own bounded
instrumentation process while candidate outcomes and timing are measured in a
separate branch-free parent process. Bounded branch evaluation does not mutate
the replayed disabled parent in the tested development cases.

**Evidence**

- `e0_controller/override_gate_v2_development.py` — development-only Stage A
  and Stage B case contracts, distinct timing semantics, killable workers, and
  parent-trace comparison
- `artifacts/override_gate/development/wp_gate_0_10/` — eight raw case records
  and hash-bound diagnosis
- `docs/E0_OVERRIDE_GATE_V2_DEVELOPMENT_ARCHITECTURE_v1.md` — architecture,
  bounded pilot, limits, and v2 freeze requirements
- focused validation: 47/47 engine and v2-prototype tests passed
- full collection check: 7,052 tests across 174 files

**Status**

✅ Architecture validated on wall-grid development seed 0. All eight cases
completed with zero timeout/error. Parent summaries and parent decision-trace
digests matched at N=100, N=500, and N=1000. The branch-free Stage-B
`margin_000` parent reproduced nine harmful overrides at N=100. This is not a
threshold or population result; a new immutable v2 sampling/inference contract
and authorization remain required.

---

### WP-GATE-0.11 — Frozen v2 experiment contract

**Claim**

The corrected experiment can be specified before outcomes with disjoint fresh
seeds, bounded outcome-blind Stage-A sampling, branch-free Stage-B parent
measurement, explicit missingness, and a joint fail-closed selection rule.

**Evidence**

- `docs/E0_OVERRIDE_GATE_CALIBRATION_INSTANCE_v2.json` — canonical immutable
  experiment instance
- `docs/E0_OVERRIDE_GATE_CALIBRATION_PREREGISTRATION_v2.md` — rationale,
  sampling, inference, timeout, missingness, selection, and authorization
  boundary
- `e0_controller/override_gate_calibration_v2.py` — non-executing validator;
  exports no fresh-split domain builder
- `artifacts/override_gate/development/wp_gate_0_11/` — cap-four feasibility
  pilot and scoped diagnosis
- focused validation: 25/25 v2 contract tests passed
- full collection check: 7,077 tests across 175 files

**Status**

✅ Design frozen, not implemented or authorized. Calibration uses new seeds
5000–5019; verification and holdout reserve 6000–6029 and 7000–7029. Stage A
samples at most four executed overrides per active-candidate seed replicate by
frozen outcome-blind hash priority. Stage B is branch/snapshot-free. No v2
fresh-split domain was instantiated and no threshold was selected.

---

### WP-GATE-0.12 — v2 no-outcome implementation

**Claim**

The frozen v2 sampler, matrix identities, artifact boundaries, and joint
selection semantics can be implemented and mechanically exercised without
constructing a fresh-split domain or exposing an outcome-producing command.

**Evidence**

- `e0_controller/override_gate_calibration_v2_sampler.py` — deterministic,
  outcome-blind cap-four sampler and hashed manifests
- `e0_controller/override_gate_calibration_v2_runner.py` — separate 144-cell
  Stage-B and 132-cell Stage-A plans, artifact contracts, and sealed dry-run
- `e0_controller/override_gate_calibration_v2_statistics.py` — complete-matrix
  validation and joint fail-closed selection
- `.github/workflows/override-gate-calibration-v2-plan.yml` — manual planning
  evidence only; no outcome command
- `artifacts/override_gate/development/wp_gate_0_12/` — no-outcome dry-run and
  diagnosis
- focused validation: 60/60 v2 contract, sampler, runner, and statistics tests
- full collection check: 7,112 tests across 178 files

**Status**

✅ No-outcome implementation complete. Synthetic full matrices verify
conservative tie-breaking and candidate-local exclusion for severe harm,
unresolved branch evidence, and clean parent timeout. The dry-run records zero
domains and outcomes. Parent/branch workers, immutable attempt consolidation,
authorization validation, and outcome workflow remain unimplemented; no v2
execution is possible yet.

---

### WP-GATE-0.13 — v2 development workers and immutable attempts

**Claim**

The branch-free Stage-B parent, exact sampled Stage-A replay, independent hard
process boundaries, and no-fallback resume semantics can be exercised without
constructing any fresh-split domain or exposing a production outcome command.

**Evidence**

- `e0_controller/override_gate_calibration_v2_workers.py` — killable workers,
  exact replay, timeout classification, atomic attempts, and consolidation
- `e0_controller/test_override_gate_calibration_v2_workers.py` — 13 boundary,
  replay, timeout, immutability, corruption, and missingness tests
- `artifacts/override_gate/development/wp_gate_0_13/` — successful seed-0 pilot
  and two immutable attempt records
- focused validation: 30 tests passed across the new workers, frozen sampler,
  and prior development-stage implementation
- collection accounting: 7,125 tests across 179 files

**Status**

✅ Development mechanics complete. The pilot produced 40 branch-free parent
decisions, 9 executed overrides, 4 deterministic samples, exact Stage-A parent
replay, and 4/4 completed pairs. No fresh split or holdout was accessed and no
gate result was produced. Authorization validation and an authorization-gated
production workflow remain unimplemented.

---

### WP-GATE-0.14 — v2 external authorization boundary

**Claim**

An external calibration-only authorization can be bound fail-closed to the
exact instance, protocol, execution commit, execution manifest, workflow,
reviewed no-outcome evidence, scope flags, and retry semantics without exposing
an outcome command or constructing a fresh-split domain.

**Evidence**

- `e0_controller/override_gate_calibration_v2_authorization.py` — canonical
  digest, execution-manifest validation, inert template, and external-record
  validator
- `e0_controller/test_override_gate_calibration_v2_authorization.py` — 24
  positive and negative boundary tests
- `artifacts/override_gate/development/wp_gate_0_14/` — non-operational review
  template and zero-domain/zero-outcome dry run
- focused authorization validation: 24 tests passed
- collection accounting: 7,149 tests across 180 files

**Status**

✅ Validator implemented, not authorized. Workflow, manifest, digest, commit,
scope, confirmation, metadata, and dangerous-flag drift fail closed. No valid
authorization record, production execution manifest, fresh-split builder, or
outcome workflow exists; no execution commit is frozen and no gate result was
produced.

---

### WP-GATE-0.15 — locked v2 calibration execution layer

**Claim**

The complete v2 calibration-only workflow can be implemented with
authorization-before-domain enforcement, separate Stage-B and Stage-A
matrices, frozen timeout boundaries, immutable attempts, and newest-only
consolidation without executing a fresh-split domain.

**Evidence**

- `e0_controller/override_gate_calibration_v2_execution.py` — execution
  manifest, authorized context, production workers, attempts, consolidation,
  and selection
- `.github/workflows/override-gate-calibration-v2-execute.yml` — manual
  authorization-gated Stage-B → consolidation → Stage-A → selection workflow
- `e0_controller/test_override_gate_calibration_v2_execution.py` — 17
  no-outcome execution-boundary tests
- `artifacts/override_gate/development/wp_gate_0_15/dry-run.json` — locked
  144/132-cell no-outcome review
...
- collection accounting: 7,166 tests across 181 files

**Status**

✅ Execution layer implemented and locked. Outcome commands exist but cannot
reach a domain without the exact external authorization, execution manifest,
workflow hash, commit, and confirmation. No operational authorization or
manifest exists; the workflow was not dispatched, and no calibration,
verification, holdout, or gate result exists.
...
The externally supplied WP-GATE-0.1 through WP-GATE-0.15 audit can be
preserved and its three documentation findings closed without changing the
frozen v2 instance, the execution implementation, or any outcome boundary.

**Evidence**

- `docs/E0_OVERRIDE_GATE_AUDIT_WP_GATE_0_15.md` — preserved audit scope,
  assertions, findings, disposition, and authorization boundary
- `docs/E0_EVIDENCE_LEDGER_v1.json` — separate v1 result, timeout-diagnosis,
  and v2 readiness claims covering WP-GATE-0.8 through WP-GATE-0.15
- `docs/E0_OVERRIDE_GATE_CALIBRATION_PREREGISTRATION_v2.md` — explicit
  post-freeze editorial correction record
- `bootstrap.json` — state restored through C331 and WP-GATE-0.15 while
  preserving the pending C328 runtime note
- Override-Gate regression: 318 passed
- collection accounting: 7,166 tests across 181 files

**Status**

✅ Audit findings closed as documentation and provenance work. No production
code, frozen instance, authorization, execution manifest, calibration,
verification, protected holdout, or gate result was changed or created.

---

### C332 — WP-2.4 evidence retention and development result

**Claim**

The complete corrected WP-2.4 development bundle from GitHub Actions run
30526724307 can be retained byte-identically before artifact expiry
(2026-08-13), and its three preregistered development diagnostics — including
the previously unreported G1-B baseline comparison — reported without opening
the holdout or changing any frozen configuration.

**Evidence**

- `artifacts/g1/E0-G1-v1/development/run_30526724307/` — retained bundle;
  all five file SHA-256 values verified against the in-bundle `manifest.json`
- `docs/E0_G1_DEVELOPMENT_RESULT_v1.md` — provenance, diagnostics, criteria
  application, and consequences
- `docs/research/E0_DECISION_BENCHMARK_v1.md` — workflow-generated report,
  committed unchanged
- `docs/E0_EVIDENCE_LEDGER_v1.json` — new claim `E0-G1-DEV-BASELINES-001`
  plus updated `E0-HIST-DOMINANCE-001`
- collection accounting: 7,166 tests across 181 files (unchanged; no new
  tests)

**Status**

✅ Retention and reporting only. `holdout_accessed=false` throughout the
bundle; no configuration, frozen instance, protocol, or execution semantics
changed; no Gate-G1 PASS/FAIL created. The G1-B development diagnostic is
adverse to A_HIST in three of four families; see the result document for the
preregistered interpretation boundary.

---

### C333 — Gate G1-v1 closure

**Claim**

The user's closure decision of 2026-08-05 can be recorded with binding
consequences (holdout permanently unopened, geometry research freeze,
WP-2.5 closed unexecuted, Override-Gate v2 permanently parked) without
touching any test, frozen instance, or outcome artifact.

**Evidence**

- `docs/E0_G1_CLOSURE_v1.md` — decision, basis, consequences, closure arc plan
- `docs/E0_EVIDENCE_LEDGER_v1.json` — decision claim `E0-GATE-G1-CLOSURE-001`
- `bootstrap.json` — gate_g1 CLOSED-NEGATIVE, P3/P4b phase states, ARC-O
- collection accounting: 7,166 tests across 181 files (unchanged)

**Status**

✅ Governance-only closure record. No holdout access, no code change, no new
outcome. The closure explicitly does not upgrade development diagnostics to a
holdout gate result.

---

## 6. Historical test-file inventory

The rows below predate the collection-authoritative count in the header and are retained as a historical claim map.
...
| `test_dream_mode.py` | 98 | C109/C110/C111/C119/C139 |
| `test_dream_node_bridges.py` | 32 | C154 |
| `test_demo_bootstrap.py` | 13 | C140/C141 |
| `test_demo_multiverse.py` | 11 | C142 |
| `test_demo_curriculum.py` | 11 | C143 |
| `test_demo_reflexion.py` | 15 | C144 |
| `test_structural_entropy.py` | 101 | C115/C116/C117/C118/C121 |
| `test_demo_self_graph.py` | 21 | C147 |
...
| `test_shared_historization.py` | 14 | C189 |
| `test_g1_domains.py` | 20 | C324 / WP-2.1 |
| `test_g1_baselines.py` | 33 | C325 / WP-2.2 |
| `test_g1_ablations.py` | 34 | C326 / WP-2.3 + C330 scaling parity |
| `test_g1_development_report.py` | 23 | C327 / WP-2.4 infrastructure |
| `test_g1_development_distributed.py` | 9 | C328 / WP-2.4 bounded distribution |
| `test_g1_timeout_enforcement.py` | 3 | C328 / WP-2.4 timeout enforcement |
| `test_g1_mechanism_diagnosis.py` | 6 | C331 mechanism neutralization |
| `test_override_gate_policy.py` | 35 | WP-GATE-0.2 policy/migration |
| `test_override_gate_calibration.py` | 23 | WP-GATE-0.3 preregistration contract |
| `test_override_gate_calibration_runner.py` | 31 | WP-GATE-0.4 execution scaffold |
| `test_override_gate_calibration_engine.py` | 39 | WP-GATE-0.5 domain/branch engine |
| `test_override_gate_calibration_pipeline.py` | 25 | WP-GATE-0.6 bounded worker/shards/resume + WP-GATE-0.8 funnel counts |
| `test_override_gate_calibration_statistics.py` | 22 | WP-GATE-0.6 frozen selection + WP-GATE-0.8 funnel validation |
| `test_override_gate_calibration_distributed.py` | 13 | WP-GATE-0.7 authorized cell distribution |
| `test_override_gate_development_pilot.py` | 8 | WP-GATE-0.9 development-only scaling/activation pilot |
| `test_override_gate_v2_development.py` | 8 | WP-GATE-0.10 development-only Stage-A/Stage-B prototype |
| `test_override_gate_calibration_v2.py` | 25 | WP-GATE-0.11 immutable v2 contract and safety guards |
| `test_override_gate_calibration_v2_sampler.py` | 9 | WP-GATE-0.12 outcome-blind hash sampler |
| `test_override_gate_calibration_v2_runner.py` | 17 | WP-GATE-0.12 stage plans, artifacts, and no-outcome dry-run |
| `test_override_gate_calibration_v2_statistics.py` | 9 | WP-GATE-0.12 joint synthetic selection semantics |
| `test_override_gate_calibration_v2_workers.py` | 13 | WP-GATE-0.13 killable replay workers and immutable attempts |
| `test_override_gate_calibration_v2_authorization.py` | 24 | WP-GATE-0.14 external authorization and execution-manifest boundary |
| `test_override_gate_calibration_v2_execution.py` | 17 | WP-GATE-0.15 locked production execution and distribution boundary |
| **Current collected total (authoritative; not the sum of historical rows)** | **7166** | **181 files; last full execution: 6840** |
